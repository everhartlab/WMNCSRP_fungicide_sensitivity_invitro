...
#Reading and subsetting data of discriminatory dose use
   ...
   #  
    ...
    survey.data.Cristian.drybeans <- 
      read.csv("data/DC_survey-WMNCSRP_Cristian.csv")
    survey.data.Cristian.drybeans <-survey.data.Cristian.drybeans %>% 
# select(ID, repeats, experimental_replicate, control.ecuatorial_30hr, control.polar_30hr, boscalid.ecuatorial.30hr, boscalid.polar_30hr) %>% 
      ...
      #   )  %>%
      mutate(
        control.growth.boscalid = ((
        control.boscalid.ecuatorial + control.boscalid.polar
        ) / 2),
        boscalid.growth = ((
        boscalid.ecuatorial + control.boscalid.polar
        ) / 2),
        
        control.growth.boscalid.37hrs = ((
        control.boscalid.ecuatorial.37hrs + control.boscalid.polar.37hrs
        ) / 2),
        boscalid.growth.37hrs = ((
        boscalid.ecuatorial.37hrs + boscalid.polar.37hrs
        ) / 2),
        sham.growth.picoxystrobin = ((sham.ecuatorial + sham.polar) / 2),
        picoxystrobin.growth = ((
        picoxystrobin.ecuatorial + picoxystrobin.polar
        ) / 2),
        control.growth.tetraconazole = ((
        control.tetraconazole.ecuatorial + control.tetraconazole.polar
        ) / 2),
        tetraconazole.growth = ((
        tetraconazole.ecuatorial + tetraconazole.polar
        ) / 2),
        control.growth.TM = ((control.TM.ecuatorial + control.TM.polar) / 2),
        TM.growth = ((
        TM.ecuatorial + TM.polar
        ) / 2),
          ID = as.character(ID), new = "Yes") %>%
          select(ID,
          repeats,
          experimental_replicate,
          boscalid.growth,
   ...
   survey.data.Cristian.drybeans.boscalid <-  survey.data.Cristian.drybeans %>% 
     select(ID,
          repeats,
          experimental_replicate,
          boscalid.growth,
          control.growth.boscalid,
          #boscalid.growth.37hrs,# taking out this for a meantime
          #control.growth.boscalid.37hrs,# taking out this for a meantime
          new) %>% 
     rename(control.growth= control.growth.boscalid)
   #Picoxystrobin Data
   
   survey.data.Cristian.drybeans.picoxystrobin <-  survey.data.Cristian.drybeans %>% 
     select(ID,
          repeats,
          experimental_replicate,
          picoxystrobin.growth,
          sham.growth.picoxystrobin,
          new) %>% 
     rename(sham.growth= sham.growth.picoxystrobin)
    #Tetraconazole Data
   survey.data.Cristian.drybeans.tetraconazole <-  survey.data.Cristian.drybeans %>% 
     select(ID,
          repeats,
          experimental_replicate,
          tetraconazole.growth,
          control.growth.tetraconazole,
          new) %>% 
     rename(control.growth= control.growth.tetraconazole)
   
   #TM Data
   survey.data.Cristian.drybeans.TM <-  survey.data.Cristian.drybeans %>% 
     select(ID,
          repeats,
          experimental_replicate,
          TM.growth,
          control.growth.TM,
          new) %>% 
     rename(control.growth= control.growth.TM)
...
boscalid_data <- survey.data.3 %>%
  select(ID,
         ...
         boscalid.growth,
         control.growth, new) %>%
  rbind(survey.data.Cristian.drybeans.boscalid) %>% 
  rename(response = boscalid.growth, control = control.growth) %>% mutate(new= as.factor(new))
#check why after the function is taking som much obeservations out, almost the middle
...
####
boscalid.complete$Field <- factor(boscalid.complete$Field, c("WI_Hancock", "MI_Montcalm_2017", "IA_Nashua","MI_Montcalm_2015", "Baseline",  "Mexico_Brazil","MI:East_Lansing"))

plot.bos.3 <- boscalid.complete%>%
  ggplot(aes(x = Field, y = EC50DC)) +
  geom_jitter(
    width = .1,
    height = 0,
...
picoxystrobin_data <- survey.data.3 %>%
  ...
  rbind(survey.data.Cristian.drybeans.picoxystrobin) %>% 
...
picoxystrobin.complete <- final.picoxystrobin.DC %>% 
  ...
  filter(Field== "MI:East_Lansing"| 
        ...
        Field== "WI_Cerny"|
         ...
         Country== "Brazil") %>% 
  arrange(EC50DC) %>% 
  mutate(Field = fct_recode(Field, "Mexico_Brazil" = "other_field"))
...
```{r, echo= FALSE}
summary(aov (EC50DC ~ Field, data = picoxystrobin.complete))
model <- lm(EC50DC ~ Field, data = picoxystrobin.complete)
ANOVA <- aov(model)
...
#There NO difference p-value = 0.4156
