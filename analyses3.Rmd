...
Survey fungicide sensitivity of populationss of S. sclerotiorum from soybean
#Backgroundddd
...
boscalid.data <-
 ...
 # Filter out the control observations that have less than 5 cm  diameter growth
...
```{r setting objects}
#replacing with the original ID, for example, instead of 12b, now 12
...
```{r function to take outliers from 8 observations}
...
fig.1<- ggplot(final.boscalid.DC.0.2, aes(x = Estimate.50, y = Estimate.50DC)) +  theme(
  plot.title = element_text(
  size = 14,
  face = "bold",
  hjust = 1,
...
##  Boscalid using the DC inlcuding Farmer fields and Fungicide field trials
```{r Using the DC Boscalid and the model}
...
##  Analysis by fungicide
#  Picoxystrobin
```{r Serial dilution Picoxystrobin}
...
#  Picoxystrobin
```{r Calculus of DC Picoxystrobin}
...
#  Picoxystrobin Using the DC including Farmer fields and Fungicide field trials
```{r Using the DC Picoxystrobinand the model}
...
##  Analysis by fungicide
#  Tetraconazole
```{r Serial dilution Tetraconazole}
...
#  Tetraconazole
```{r Calculus of DC Tetraconazole}
#Getting relative growth column at each dose where RG= Relative Growth, eg. RG0.01= Relative Growth at 0.01 ppm
...
#  Tetraconazole using the DC and the model including Farmer fields and Fungicide field trials
```{r Using the DC Tetraconazoleand the model}
...
##  Analysis by fungicide
#  Thiophanate methyl
```{r Serial dilution Thiophanate methyl}
...
##  Procedure to get the Discrimanatory Concentration (DC) 
#  Thiophanate methyl
```{r Calculus of DC Thiophanate methyl}
#Getting relative growth column at each dose where RG= Relative Growth, eg. RG0.01= Relative Growth at 0.01 ppm 
...
## Linear Regression graph of EC50 and EC50DC
fig.4 <- ggplot(final.TM.DC.10, aes(x = Estimate.50, y = Estimate.50DC)) +  theme(plot.title = element_text(size = 16, face = "bold", hjust = 1)) + labs(title = "Linear regression of thiophanate methyl 10 ppm", x ="EC50 (ppm)", y = "EC50DC (ppm)" ) + geom_point() + geom_smooth(method = "lm")
...
#  Thiophanate methyl Using the DC  including Farmer fields and Fungicide field trials
```{r Using the DC Thiophanate methyl and the model}
...
#  Another approach with TM to pick the probably isolates with high EC50 using a new model of 2.5 ppm, the closest at the EC50
```{r Selecting outliers of the serial dilution Thiophanate  methyl }
...
#  Another approach with TM to pick the probably isolates with low sensitivity based on the growth at Discrimantory concentrartion,including farmer fields and fungicide field trials 
```{r Estimating low sensitivity TM}
...
##  Wrangling datframe for each fungicide
```{r wrangling fungicides}
...
# Function to detect outliers considered since the third quantil from the outliers of ecah fungicicde

```{r function to detect outliers includng third quantil}
...
knitr::opts_chunk$set(echo = FALSE)
```
# Detection of  outliers (third quantil) using the previous function
...
mu <- boscalid.complete.2 %>%
  full_join(WM3_, by = "ID") %>% # selecting variables more important 
   ...
   left_join(summary.field.original)  %>% # joining with table of summary of how many isolates are per field
...
TM.complete.2<- TM.complete%>% #this the Id of serial dilution
    bind_rows(TM.data) %>% # adding the selection of isolates when the DC (the hhishest was used 10 ppm) Whatch out here is much more the really tested that is why it was created another object TM complete
ungroup() %>% 
  mutate( ID= as.numeric(ID)) %>% 
  select(ID) %>% 
    distinct() # # corroborrating any duplication in ID
  # anyDuplicated(TM.complete.2$ID)

####
TM_data.2 <- TM_data %>%  #fisrt I need to do that for beter binding
  select(ID)
#This is to be used in NA,0 and 1

TM.complete.3<- TM.complete%>% 
  select(ID) %>% 
  bind_rows(TM_data.2) %>% 
  ungroup() %>% 
  mutate( ID= as.numeric(ID)) %>% 
...
notoutlierbuttested_TM <-TM.complete.3$ID[! TM.complete.3$ID %in% TM.complete.2$ID  ]
...
ID%in% boscalid.complete.2$ID ~ 1,
ID%in% notoutlierbuttested_bos~ 0,
TRUE ~ 3), # I used this "NA_real_" instead of "NA" becasue Iam getting this error: must be a double vector, not a logical vector but now IAM USING instead 3 becasue i could not filter then NA's
...
TRUE ~ 3)) %>%
  ...
  arrange(Group) %>% 
  mutate(bos = as.character(bos), pico = as.character(pico), tetra = as.character(tetra), TM = as.character(TM) )
...
hello <- Compendium %>%  select(  bos, pico, tetra, TM)
#bye <- apply(hello, 1, sum)
#water <- cbind(hello, bye)
#filtering to know how many isolates coincide to be  outliers for at at least 3 fungicicdes
#fusarium <- water %>% filter(bye>=3) %>% arrange(bye) 

#datfarmae for select how many of those to use for test
hola <- Compendium %>% 
   filter(N_iso_perfield >= 5) %>% #filtering those greater or eauql than 5 isolates/field
  filter(bos ==3, pico==3, tetra == 3, TM==3 ) %>% #filter all NO tested, NA or 3
    group_by(Group, Field) %>% 
  select(-c( bos, pico, tetra, TM)) %>% 
mutate(N_iso_tested= n()) %>%# countig those tested
    mutate(N_iso_perfield = as.numeric(N_iso_perfield)) %>% # converting to numeric
  mutate(N_iso_tested= N_iso_perfield-N_iso_tested) %>% #subtracting
      mutate(N_iso_to_pick = ifelse(N_iso_perfield >= 10, 10,  N_iso_perfield  )) %>% #if there is more than 10 converting to t10 if is less than 10, keoin the original number
  mutate(N_iso_to_pick = N_iso_to_pick - N_iso_tested  )%>% #subtracting
    select(-c(Host, Origin)) %>% 
   mutate(N_iso_to_pick_EX =  
           ifelse( N_iso_perfield  >= (N_iso_to_pick +  3), (N_iso_to_pick + 3), ifelse( N_iso_perfield  >= (N_iso_to_pick +  2), (N_iso_to_pick + 2), N_iso_to_pick ))) # adding more extra isolates for the case many times they do not grow from the sclerotia 
  

 
#dataframe the real total to use
hola.2 <- hola %>%  
  sample_n(size = N_iso_to_pick) %>% #randomly sampling
       arrange(Group) %>% 
  rename(Nperfield = N_iso_perfield , Ntested = N_iso_tested,Nselect  = N_iso_to_pick, Nextra= N_iso_to_pick_EX)
 
     
#dataframe with the extra taking in case the sclerotia does not growth
hola.3 <- hola %>%   
  sample_n(size = N_iso_to_pick_EX, replace = T) %>% 
   #randomly sampling
  arrange(ID) %>% rename(Nperfield = N_iso_perfield , Ntested = N_iso_tested,Nselect  = N_iso_to_pick, Nextra= N_iso_to_pick_EX)
...
picoxystrobin.complete$ID[!picoxystrobin.complete$ID%in% boscalid.complete$ID]
