...
boscalid.data <-
 ...
 # Filter out those doses that not have 2nd experimental replication or they were not needed according to the model
...
TMdata$ID <- as.numeric(TMdata$ID)   
  #Filter out the control observations something weird happened with the repeats 3 so Iam revoming those
    TMdata <- TMdata %>%
      group_by(ID, experimental_replicate, dose, ecuatorial, polar, repeats) %>%
...
#Reading and subsetting data of discriminatory dose use
  ...
  #Discriminatory dose boscalid
   # its by default make at 30 hrs
    survey.data.boscalid <- 
      read.csv("data/DD_survey-WMNCSRP_boscalid.csv")
    survey.data.boscalid <-survey.data.boscalid %>% 
select(ID, repeats, experimental_replicate, control.ecuatorial, control.polar, boscalid.ecuatorial, boscalid.polar) %>% 
      # group_by(
      #   ID,
      #   repeats,
      #   experimental_replicate,
      ...
      #   )  %>%
        mutate(
    control.growth = ((control.polar + control.ecuatorial) / 2),
    boscalid.growth = ((
...
# reading the inventory Sclerotinia collection with some of my update modifications
                            WM <- read.csv("data/WM-fixingproductnamesID2573_modifyingforactiveingredient_11.csv", stringsAsFactors = TRUE)
WM$Collection_ID <- str_squish(WM$Collection_ID)
WM[] <- lapply(WM, function(x) {
    is.na(levels(x)) <- levels(x) == "NA"
    x
})
#Still not working for NA
WM[WM == ""] <- NA

sum(is.na(WM$Year))
# 18
sum(is.na(WM$Country))
# 3
sum(is.na(WM$County))
#323
sum(is.na(WM$Field))
# 863
# need to ckec which isolates have been done with Kamvar article and ANthony Panullo in supplementary data
...
levels(WM3$Country)[levels(WM3$Country) == ""] <- "unknown"

levels(WM3$State)[levels(WM3$State) == ""] <- "Unknown"

levels(WM3$County)[levels(WM3$County) == ""] <- "unknown"
...
fig.1.boscalid <-
  ...
  ) + geom_smooth(method = "lm", se = FALSE, colour= "black") +  theme(
  plot.title = element_text(
  size = 9,
  face = "bold",
  hjust = 0.5,
  ...
  panel.background = element_rect(fill = "white", colour = "grey50")
              ...
              "Relative growth (%)", y = expression(bold(Log  (EC[bold("50")])))) + expand_limits(x = c(15, 65)) + scale_x_continuous(name = waiver(), breaks = c(15, 25, 35, 45, 55, 65)) + scale_y_continuous(breaks = c(-3, -2.75, -2.5, -2.25, -2.0, -1.75, -1.5))
...
expression(bold(EC[bold("50")])~( bold(ppm ~bold("a.i.")))), y = expression(bold(EC[bold("50")~(bold("D"))])~( bold(ppm ~bold("a.i.")))))+ geom_abline(
                ...
                lty = "dashed",
                size = 0.3
                )+  expand_limits(x = c(0.04, 0.25), y= c(0.04, 0.25))      + scale_x_continuous(name = waiver(), breaks = c(0.05, 0.1, 0.15, 0.20, 0.25)) + scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.20, 0.25))
...
ggplot(final.boscalid.DC.two, aes(x = Estimate.50, y = Estimate.50DC)) +  geom_point(aes(
   shape = Host,
   colour = source,
   fill = Host
 ),
 size = 2,
 alpha = 3 / 4) +
   scale_color_grey(start = 0, end = 1) + scale_shape_manual(values = c(15, 16, 17)) +
   ...
   expand_limits(x = c(0.04, 0.25), y = c(0.04, 0.25))  + scale_x_continuous(name = waiver(),
...
###For isolates from that have reduced sensitivity, assess sensitivity for isolates in the same field to account for heterogeneicity. These fields/plots were also selected based on whether or not there was a single mode-of-action fungicide applied in the current year (N= 14) 
...
```{r}
fig.1.picoxystrobin <-
  ggplot(finalRG0.01, aes(x = RG0.01, y = logEC50)) +  geom_point(aes(shape= Host, colour = source, fill= Host),
                                                                                          size = 2,
                                                                                  alpha = 3 / 4
) + 
    scale_color_grey(start = 0,end = 1) + scale_shape_manual(values = c(15, 16, 17)) + 
  ...
  panel.background = element_rect(fill = "white", colour = "grey50")
              ...
              "Relative growth (%)", y =  expression(bold(Log  (EC[bold("50")])))) + expand_limits(x = c(15, 65)) + scale_x_continuous(name = waiver(), breaks = c(15, 25, 35, 45, 55, 65)) 
...
expression(bold(EC[bold("50")])~( bold(ppm~bold("a.i.")))), y = expression(bold(EC[bold("50")~(bold("D"))])~( bold(ppm~bold("a.i."))))) + geom_abline(
                ...
                lty = "dashed",
                lwd = 1
 ) + expand_limits(x = c(0.0060, 0.018), y = c(0.0060, 0.018)) + scale_x_continuous(name = waiver(),
breaks = c(0.0075, 0.01, 0.0125, 0.015,  0.018)) + scale_y_continuous(name = waiver(),
breaks = c(0.0075, 0.01, 0.0125, 0.015,  0.018))
...
ggplot(final.picoxystrobin.DC.two,
       aes(x = Estimate.50, y = Estimate.50DC)) +  geom_point(aes(
         shape = Host,
         colour = source,
         fill = Host
       ),
...
final.tetraconazole.DC.two <-
 final.tetraconazole.DC %>% 
 mutate(source = ifelse(
    ID %in% baseline_isolates_2,
    "Baseline",
    ifelse(
...
ggplot(final.tetraconazole.DC.two,
       aes(x = Estimate.50, y = Estimate.50DC)) +  geom_point(aes(
         shape = Host,
         colour = "black",
         fill = source
       ...
       size = 2,
       alpha = 3 / 4) +     scale_color_grey(start = 0, end = 1) +   scale_shape_manual(values = c(21, 22, 24))+  geom_smooth(method = "lm", se = FALSE, color = "black") +
  theme(
    plot.title = element_text(
      size = 10,
      face = "bold",
  ...
  expand_limits(x = c(0.3, 1.8), y = c(0.3, 1.8)) + scale_x_continuous(breaks = c(0.3, 0.6, 0.9, 1.2, 1.5, 1.8)) +
  scale_y_continuous(breaks = c(0.3, 0.6, 0.9, 1.2, 1.5, 1.8))
...
picoxystrobin.filtered. <- picoxystrobin.filtered.survey %>%
  ...
  mutate(ID = as.numeric(ID)) %>%
   select(c(ID, EC50DC)) %>% 
mutate(new = ifelse(
    ID %in% picoxystrobin.filtered.survey.yes,"Yes","No"))
...
```{r}
# casa
save(list = ls(all=TRUE), file= "Main.Rdata")
 rm(list = ls())
