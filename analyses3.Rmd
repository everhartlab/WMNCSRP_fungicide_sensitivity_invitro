...
select(ID, repeats, experimental_replicate, control.ecuatorial, control.polar, tetraconazole.ecuatorial, tetraconazole.polar) %>% 
      # group_by(
      #   ID,
      #   repeats,
      #   experimental_replicate,
    ...
    tetraconazole.ecuatorial + tetraconazole.polar
      ...
      select(ID, repeats, experimental_replicate, control.growth, tetraconazole.growth, new)
...
select(ID, repeats, experimental_replicate, control.ecuatorial, control.polar, TM.ecuatorial, TM.polar) %>% 
      # group_by(
      #   ID,
      #   repeats,
      #   experimental_replicate,
    ...
    control.growth = ((control.ecuatorial + control.polar) / 2),
    TM.growth = ((
    TM.ecuatorial + TM.polar
    ) / 2), ID = as.character(ID), new= "Yes") %>% 
      select(ID, repeats, experimental_replicate, control.growth, TM.growth, new)
...
```{r}
fig.1.boscalid <-
  ggplot(finalRG0.2, aes(x = RG0.2, y = logEC50)) +  geom_point(
  shape = 21,
  fill = "black",
  size = 2,
  alpha = 3 / 4
...
fig.2.boscalid<- ggplot(final.boscalid.DC.0.2, aes(x = Estimate.50, y = Estimate.50DC)) +  geom_point(
  shape = 21,
  fill = "black",
  size = 2,
  alpha = 3 / 4
...
#Selecting the EC50D from the serial dilution
boscalid.semi.complete <- final.boscalid.DC %>% 
 ####WATCH OUT, Based on EC50DC
   rename(EC50DC = Estimate.50DC) %>% 
  select(ID, EC50DC) %>% 
...
boscalid.complete.fields <- boscalid.complete%>% 
  filter(Field== "MI:East_Lansing"| 
         ...
         Country== "Brazil") %>% 
  arrange(EC50DC) %>% 
mutate(Field = fct_recode(Field, "Mexico_Brazil" = "other_fields"))
...
# by Field  kruskal wallis test 
There is NO difference, P value is 0.1686
```{r}
shapiro.test.boscal.fields <- boscalid.complete.fields %>%
   do(tidy(shapiro.test(.$EC50DC)))
shapiro.test.boscal.fields
# Almost but not 0.0450
object.3 <- kruskal.test(EC50DC ~ Field, data = boscalid.complete.fields)
object.3[[3]][[1]] 
#There is NO difference, P value is 0.1686772
```
# by Host  kruskal wallis test 
There is NO difference, P value is 0.99
...
shapiro.test.boscal.fields
# Almost but not 0.0450
object.4 <- kruskal.test(EC50DC ~ Host, data = boscalid.complete.fields)
object.4[[3]][[1]] 
...
shapiro.test.boscal.fields
# Almost but not 0.0450
object.5 <- kruskal.test(EC50DC ~ source, data = boscalid.complete.fields)
object.5[[3]][[1]] 
...
####
boscalid.complete.fields$Field <- factor(boscalid.complete.fields$Field, c("WI_Hancock", "MI_Montcalm_2017", "IA_Nashua","MI_Montcalm_2015", "Baseline",  "Mexico_Brazil","MI:East_Lansing"))
...
# Linear Regression graph of log EC50 and relative growth at 0.2 ppm
 ...
 fig.1 <-    ggplot(finalRG0.01, aes(x = RG0.01, y = logEC50)) +  geom_point() + geom_smooth(method = "lm") +  theme(
    plot.title = element_text(
    size = 8,
    face = "bold",
    hjust = 0.5,
...
```{r}
fig.1.picoxystrobin <-
  ggplot(finalRG0.01, aes(x = RG0.01, y = logEC50)) +  geom_point(
  shape = 23,
  fill = "black",
  size = 2,
  alpha = 3 / 4
...
fig.2.picoxystrobin <- ggplot(final.picoxystrobin.DC.0.01,
       aes(x = Estimate.50, y = Estimate.50DC)) + 
    geom_point(
  shape = 23,
  fill = "black",
  size = 2,
...
object.6 <- kruskal.test(EC50DC ~ source, data = picoxystrobin.complete)
object.6[[3]][[1]]

#There IS difference p-value = 0.001368

object.7 <-  DunnTest(EC50DC ~ source, data = picoxystrobin.complete, method = "bonferroni")
...
shapiro.test.pico 
#NO NORMALILTY  , thats is why  krusKal
object.8 <- kruskal.test(EC50DC ~ Host, data = picoxystrobin.complete)
object.8[[3]][[1]]
#There is difference between host ,  0.044
object.9 <-  DunnTest(EC50DC ~ Host, data = picoxystrobin.complete, method = "bonferroni")
object.9 

#Drybean-different_host       1.0000    
# Soybean-different_host       1.0000    
# Soybean-Drybean              0.0382 *
```
...
# in case not NORMAL
object.10 <- kruskal.test(EC50DC ~ Field, data = picoxystrobin.complete.fields)
object.10[[3]][[1]]
#There is difference 0.001431

object.11 <- DunnTest(EC50DC ~ Field, data = picoxystrobin.complete.fields, method = "bonferroni")

# there are differenets
#Baseline-Mexico_Brazil   0.0088 **
#Baseline-MI_Montcalm_2015 0.0673  . 
#Baseline-WI_Hancock       0.0108 *

#by host
object.12 <- kruskal.test(EC50DC ~ Host, data = picoxystrobin.complete.fields)
object.12[[3]][[1]]
#There is  no difference 0.1599

#by source
object.13 <- kruskal.test(EC50DC ~ source, data = picoxystrobin.complete.fields)
object.13[[3]][[1]]
#There is   difference 0.001414

object.14 <- DunnTest(EC50DC ~ source, data = picoxystrobin.complete.fields, method = "bonferroni")
object.14
# FF-Baseline       0.0166 *  
# FFT-Baseline     0.0012 ** 
# FFT-FF           1.0000  
# in case  NORMAL distributed
shapiro.test.pico.3 <-picoxystrobin.complete.fields %>%
  group_by(Field) %>%
  do(tidy(shapiro.test(.$EC50DC)))
shapiro.test.pico.3
#There are normal distributed that is why ANOVA
AOV <- (aov (EC50DC ~ Field, data = picoxystrobin.complete.fields))
model <- lm(EC50DC ~ Field, data = picoxystrobin.complete.fields)
ANOVA <- aov(model)
tukey.test <- TukeyHSD(ANOVA)
tukey.test.2 <- HSD.test(AOV, trt = 'Field' )
HSD.test(model, "Field")
summary(model)
summary(ANOVA)
# in case not NORMAL
kruskal.test(EC50DC ~ Field, data = picoxystrobin.complete)
#There is difference 0.001431

DunnTest(EC50DC ~ Field, data = picoxystrobin.complete, method = "bonferroni")
...
##
picoxystrobin.complete$Field <- factor(picoxystrobin.complete$Field, c("Mexico_Brazil","MI_Montcalm_2015", "WI_Hancock", "MI:East_Lansing", "MI_Montcalm_2017", "IA_Nashua", "WI_Cerny",  "Baseline"))
...
object.14 <- kruskal.test(EC50DC ~ source, data = tetraconazole.complete)
object.14[[3]][[1]]
...
object.15 <- kruskal.test(EC50DC ~ Host, data = tetraconazole.complete)
object.15[[3]][[1]]
...
object.16 <- kruskal.test(EC50DC ~ Field, data = tetraconazole.complete.fields)
object.16[[3]][[1]]
#There is YES difference p-value 0.008304
object.17 <- DunnTest(EC50DC ~ Field, data = tetraconazole.complete.fields)
object.17
#No differenec between ranks due to no any value less or equal to 0.05
#by host

object.18 <- kruskal.test(EC50DC ~ Host, data = tetraconazole.complete.fields)
object.18[[3]][[1]]

# P values is 0.0242661, there IS diferenec between hosts

#by source

object.19 <- kruskal.test(EC50DC ~ source, data = tetraconazole.complete.fields)
object.19[[3]][[1]]
#Thre is NO differnece teween source, P value is 0.1383376
...
##
tetraconazole.complete.fields$Field <- factor(tetraconazole.complete.fields$Field, c("NE:Field_34","NE:Field_67","NE:Field_101","NE:Field_98", "NE:Field_97", "Baseline", "NE:Field_75", "Mexico_Brazil"))
