...
#  Analysis of Picoxystrobin fungicide
## Serial dilution   
...
#  Analysis of Tetraconazole fungicide
##  Serial dilution
...
num_isolates_selected <- fields_more_4 %>% 
  ...
  mutate(N_iso_not_tested= n()) %>%# countig those tested
  mutate(N_iso_tested= N_iso_perfield-N_iso_not_tested) %>% #subtracting
  mutate(N_iso_to_pick = ifelse(N_iso_perfield >= 10, 10,
                                N_iso_perfield  )) %>% #if there is more than 10 converting to t10 if is less than 10, keoin the original number
...
selected_isolates <- num_isolates_selected %>%
  left_join(select_max_iso_field) %>% 
  ungroup() %>% 
  group_by(Field) %>% 
  sample_n(size = Nmax_per_field) %>% #randomly sampling
  arrange(Group) %>% 
...
final_data_isolates <- id_selected_isolates %>% 
  left_join(info_selected_isolates) %>% 
  arrange(Group) %>% 
  group_by(Field, Group) %>% 
