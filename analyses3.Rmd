...
#  Boscalid 
```{r Calculus of DC Boscalid, echo = FALSE, results= "asis"}
...
#  Picoxystrobin
```{r Calculus of DC Picoxystrobin, echo = FALSE, results= "hide"}
...
# because we did not get any possible outlier then..
...
```{r wrangling fungicides, echo = FALSE}
...
#Boscalid
# Using the function to detect outliers from all tested
...
# Using the function fo dtect oulires from serial dilution grouping by origin
...
#This "TM.complete.3" can be a total of outliers
...
###### Anoher  approach for to be used in NA,0 and 1 becuase there were the really tested inlcuding DC teste
#fisrt I need to do that for beter binding fo use later
TM_data.2 <- TM_data %>%  
  select(ID)

#This is now to  to be used in NA,0 and 1

TM.complete.4<- TM.complete%>% 
  select(ID) %>% 
  bind_rows(TM_data.2) %>% 
  ungroup() %>% 
  mutate( ID= as.numeric(ID)) %>% 
...
casa <-WM3_ %>% # selecting variables more important 
   ...
   mutate(Group_MOA_current_season = na_if(Group_MOA_current_season, "")) %>%
    ...
    mutate(Origin = as.factor(Origin), Host= as.factor(Host), Field= as.factor(Field) ) %>% filter(Group== "Group_one"|Group=="Group_3"|Group=="Group_7"|Group=="Group_11")%>% 
    mutate(Group = recode(Group, Group_one = 1, Group_3 =  3, Group_7 = 7, Group_11 = 11)) %>% 
group_by(Group, State, Year, Origin, Field) %>% 
  summarize(Total = n()) %>% 
  filter(Total >= 5 ) %>% 
     arrange(Group,Year) 
