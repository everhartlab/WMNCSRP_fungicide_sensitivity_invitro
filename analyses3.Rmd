...
ggplot(aes(x = group, color= source)) +
geom_density()
...
boscalid.complete.groups %>%
  ...
  summarize(N=n()) %>% 
  ggplot(aes(x = group, y= N,  color= source, fill= source) ) +
...
num_isolates_selected <- fields_more_4 %>% 
  ...
  mutate(N_iso_to_pick = ifelse(N_iso_perfield >= 10, 10,
                                N_iso_perfield  )) %>% #if there is more than 10 converting to t10 if is less than 10, keoin the sourceal number
...
SUMMARY.final_data_isolates <- final_data_isolates %>% 
 ...
 ##For boscalid test
  ...
  final_data_isolates %>% filter(Group == 7) %>% arrange(ID) %>% kable(format = "markdown",caption = "Table with outliers for each fungicide, NA= never tested, 0= not an outlier but tested, 1= outlier", padding = 0, align = "l") %>%
...
WM4 <- WM3_ %>%
  mutate(Fungicide_status= ifelse(
    ...
    ID %in% id_selected_isolates$ID, #From the real selected to test
    "selected", # its better to keep it separate and taht form wI cand differentiate, when I need to filter it should be, == No_tested& selected/no_selected 
    "no_selected") ) %>% 
