...
baby <- picoxystrobin.complete$ID[duplicated(picoxystrobin.complete$ID)]

#now I will take these repeats from the DC test coming

picoxystrobin.filtered.2 <- picoxystrobin.filtered. %>% filter(!ID %in% baby )
...
baby <- tetraconazole.complete$ID[duplicated(tetraconazole.complete$ID)]

#now I will take these repeats from the DC test coming

tetraconazole.filtered.2 <- tetraconazole.filtered. %>% filter(!ID %in% baby )
...
mu <- boscalid.complete.5 %>%
  full_join(WM3, by = "ID") %>%
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(boscalid.complete.5), Field,  Year, State, molecule.s._current_season, Host) %>%
...
no_tested <-mu$ID[! mu$ID %in% boscalid.complete.4$ID ]
