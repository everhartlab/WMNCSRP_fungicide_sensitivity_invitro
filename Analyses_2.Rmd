...
zz1 <- EC_table(TM.filtered, form = response ~ dose)
...
#Procedure to get Discriminatory conecntration
 ...
 RG <- TM.filtered %>% group_by(ID,dose) %>% summarise(mean_response=mean(response,na.rm=TRUE)) %>% spread(dose,mean_response) %>%  mutate(RG0.75=((`0.75`/`0`)*100)) %>% mutate(RG1=((`1`/`0`)*100)) %>% mutate(RG1.5=((`1.5`/`0`)*100)) %>% mutate(RG2=((`2`/`0`)*100)) %>% mutate(RG2.5=((`2.5`/`0`)*100)) %>% 
...
# Getting the EC50DD according to the model
final.TM.DD1 <- final.TM1 %>% mutate(Estimate.50DC = exp(finalRG2.5.1[[1]][[1]] + finalRG2.5.1[[1]][[2]]*RG2.5)) %>% # here I need to do this dynamic
  select(ID, Estimate.50DC, Estimate.50)
...
#  Becasue of that taking outlayers by and ##USind the model
...
final.boscalid.DC_2 <- final.boscalid.DC %>% 
  rename(EC50DC = Estimate.50DC) %>% 
  select(ID, EC50DC) 
####WATCH OUT, Based on EC50DC

boscalid.complete <-  rbind.data.frame(final.boscalid.DC_2, boscalid.filtered.)

boscalid.complete <- boscalid.complete %>% 
ungroup(boscalid.complete) %>%  mutate( ID= as.numeric(ID))
...
boscalid.filtered.2 <- boscalid.filtered. %>% filter(!ID %in% baby )

#Now substitute
boscalid.complete.2 <-  rbind.data.frame(final.boscalid.DC_2, boscalid.filtered.2)
##look for repetition#
...
#Selecting the EC50D from the serial dilution
final.picoxystrobin.DC_2 <- final.picoxystrobin.DC %>% 
  rename(EC50DC = Estimate.50DC) %>% 
  select(ID, EC50DC) %>% ungroup() # that is the diffrenece
####WATCH OUT, Based on EC50DC

picoxystrobin.complete <-  rbind.data.frame(final.picoxystrobin.DC_2, picoxystrobin.filtered.)

picoxystrobin.complete <- picoxystrobin.complete %>% 
ungroup(picoxystrobin.complete) %>%  mutate( ID= as.numeric(ID))


ball <- picoxystrobin.complete$ID[duplicated(picoxystrobin.complete$ID)]

#now I will take these repeats from the DC test coming

picoxystrobin.filtered.2 <- picoxystrobin.filtered. %>% filter(!ID %in% ball )

#Now substitute
picoxystrobin.complete.2 <-  rbind.data.frame(final.picoxystrobin.DC_2, picoxystrobin.filtered.2)
##look for repetition#
...
#######TETRACONAZOLE##
 ...
 #Selecting the EC50D from the serial dilution
final.tetraconazole.DC_2 <- final.tetraconazole.DC %>% 
  rename(EC50DC = Estimate.50DC) %>% 
  select(ID, EC50DC) %>% ungroup() # that is the diffrenece
####WATCH OUT, Based on EC50DC

tetraconazole.complete <-  rbind.data.frame(final.tetraconazole.DC_2, tetraconazole.filtered.)

tetraconazole.complete <- tetraconazole.complete %>% 
ungroup(tetraconazole.complete) %>%  mutate( ID= as.numeric(ID))


pool <- tetraconazole.complete$ID[duplicated(tetraconazole.complete$ID)]

#now I will take these repeats from the DC test coming

tetraconazole.filtered.2 <- tetraconazole.filtered. %>% filter(!ID %in% pool )

#Now substitute
tetraconazole.complete.2 <-  rbind.data.frame(final.tetraconazole.DC_2, tetraconazole.filtered.2)
##look for repetition#
...
##TM#######
    #Selecting the EC50D from the serial dilution
final.TM.DC_2 <- final.TM.DD1 %>% 
  rename(EC50DC = Estimate.50DC) %>% 
  select(ID, EC50DC) %>% ungroup()
####WATCH OUT, Based on EC50DC

TM.complete <-  rbind.data.frame(final.TM.DC_2, TM.filtered.)

TM.complete <- TM.complete %>% 
ungroup(TM.complete) %>%  mutate( ID= as.numeric(ID))


halo <- TM.complete$ID[duplicated(TM.complete$ID)]

#now I will take these repeats from the DC test coming

TM.filtered.2 <- TM.filtered. %>% filter(!ID %in% halo )

#Now substitute
TM.complete.2 <-  rbind.data.frame(final.TM.DC_2, TM.filtered.2)
##look for repetition#
...
kruskal.test(EC50DC ~ Origin, data = boscalid.complete.4)

#There is difference p-value = 1.25 *10 -05

DunnTest(EC50DC ~ Origin, data = boscalid.complete.4, method = "bonferroni")
...
kruskal.test(EC50DC ~ Host, data = boscalid.complete.4)
...
kruskal.test(EC50DC ~ Origin, data = picoxystrobin.complete.4)
...
kruskal.test(EC50DC ~ Host, data = picoxystrobin.complete.4)
...
kruskal.test(EC50DC ~ Origin, data = tetraconazole.complete.4)
...
kruskal.test(EC50DC ~ Host, data = tetraconazole.complete.4)
#There is NO difference p-value 0.4241


TM.complete.4 <- TM.complete.3%>% filter(!ID == 8,!ID == 129 )
...
mu <- boscalid.complete.5 %>%
  full_join(WM3_, by = "ID") %>%
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(boscalid.complete.5), Field,  Year, State, Group_MOA_current_season, Host) %>%
   left_join(summary.field.original)  %>%
   mutate(Group_MOA_current_season = na_if(Group_MOA_current_season, "")) %>%
     arrange(ID) %>% 
   
   rename(N_iso_perfield =Num_original_isolates) %>%   select(-EC50DC) 
...
picoxystrobin.complete.5<- picoxystrobin.complete.4%>% 
group_by(Origin)%>%
mutate(EC50DC_outlier = list(get_range2(EC50DC)))%>%
unnest() %>% 
filter(EC50DC >=  outlier) %>% ungroup() %>% 
...
alpha <- picoxystrobin.complete.5 %>%
  full_join(WM3_, by = "ID") %>%
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(picoxystrobin.complete.5), Field,  Year, State, Group_MOA_current_season, Host) %>%
   left_join(summary.field.original)  %>%
   mutate(Group_MOA_current_season = na_if(Group_MOA_current_season, "")) %>%
     arrange(ID) %>% 
...
tetraconazole.complete.5<- tetraconazole.complete.4%>% 
group_by(Origin)%>%
mutate(EC50DC_outlier = list(get_range2(EC50DC)))%>%
unnest() %>% 
filter(EC50DC >=  outlier) %>% ungroup() %>% 
...
gamma <- tetraconazole.complete.5 %>%
  full_join(WM3_, by = "ID") %>%
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(tetraconazole.complete.5), Field,  Year, State, Group_MOA_current_season, Host) %>%
   left_join(summary.field.original)  %>%
   mutate(Group_MOA_current_season = na_if(Group_MOA_current_season, "")) %>%
     arrange(ID) %>% 
...
angle <- TM.complete.5 %>%
  full_join(WM3_, by = "ID") %>%
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(TM.complete.5), Field,  Year, State, Group_MOA_current_season, Host) %>%
   left_join(summary.field.original)  %>%
   mutate(Group_MOA_current_season = na_if(Group_MOA_current_season, "")) %>%
     arrange(ID) %>% 
