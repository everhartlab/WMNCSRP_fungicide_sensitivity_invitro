...
```{r}
panal <- whitemold.inventory %>% mutate(source = ifelse(
  ID %in% baseline.isolates.2,
  "Baseline",
  ifelse(
    ID %in% Farmer.fields,
...
erica_hermosa <- panal %>% filter(!source == "Baseline") %>% 
  group_by(
    source,
    Host,
    country,
...
erica_hermosa.2<- panal %>% filter(!source == "Baseline") %>% filter(source == "Fungicide Field Trials") %>% 
  group_by(
    source,
    Host,
    country,
...
erica_hermosa.3<- panal %>% filter(!source == "Baseline") %>% filter(source == "Producer Fields") %>% 
  group_by(
    source,
    Host,
    country,
...
survey.tetraconazole.complete %>% filter(source=="Baseline") %>% mutate(Baseline= ifelse(
    ...
    "White mold screening nursery", "True baseline")) %>%   ggplot(aes(x = Baseline, y = EC50DC,)) + geom_violin() + geom_jitter(aes(shape= Baseline))+   theme(
...
ggarrange(a.tet.baseline,b.bos.baseline,c.pico.baseline, ncol = 1,  align = "v")
...
aaa.tet <-
  myplot_comparison(survey.tetraconazole.fields.2, highlabel = 0.1)  + expand_limits(y = c(0.4, 2.5)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2, 2.2, 2.4)) + theme(plot.margin = unit(c(1, 1, 1, 1), "cm")) +   theme(
    axis.title = element_blank(),
    axis.title.y = element_blank(),
    legend.position = "none"
  ) + theme(plot.margin = unit(c(1, 1, 1, 1), "cm")) + labs(tag = "A")  +     theme(plot.tag = element_text(face = "bold")) 
aaa.tet

bbb.bos <-
  myplot_comparison(survey.boscalid.fields.2, highlabel = 0.01) + expand_limits(y = c(0.05, 0.22)) +
  scale_y_continuous(breaks = c(0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.22)) + geom_hline(yintercept =  boscalid.less.sensive$EC50DC[9]
                                                                                                  , linetype = "dashed") +  theme(plot.margin = unit(c(1, 1, 1, 1), "cm")) +   theme(axis.title.x   = element_blank(), axis.title.y = element_blank(), legend.position = "none") +  theme(plot.margin = unit(c(-0.7, 1, 1, 1), "cm")) + labs(tag = "B")  +     theme(plot.tag = element_text(face = "bold")) # make dynamic the value of intercept, create a new fucntion
bbb.bos
