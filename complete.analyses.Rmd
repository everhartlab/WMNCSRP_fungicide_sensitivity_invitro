...
getting_DC <- function(filename){
  ...
  spread(dose, mean_response) %>% 
    rename(control = `0`) %>% #taking out "different host" becuse does not belong to drybean nor soybean
  filter(!ID == 8, !ID == 129) 
...
boscalid.EC50.2 <- boscalid.EC50 %>% select(ID, EC50) %>% 
  rename(EC50DC = EC50)#just rebaming in purpose to join the following dataframe
...
#wranglind_DC_2
survey.boscalid.2 <- survey.boscalid%>% 
  wrangling_DC() %>% 
  wrangling_DC_2()
  # count(Field_Year) %>% 
  #  filter(n >= 9)
...
survey.boscalid.2.host <- survey.boscalid.2 %>% 
  filter(!source=="Baseline")
...
#wranglind_DC_2
survey.tetraconazole.2 <- survey.tetraconazole%>% 
  wrangling_DC() %>% 
  wrangling_DC_2()
  # count(Field_Year) %>% 
  #  filter(n >= 9)
...
survey.tetraconazole.2.host <- survey.tetraconazole.2 %>% 
  filter(!source=="Baseline")
...
survey.picoxystrobin.2 <- survey.picoxystrobin%>% 
  wrangling_DC() %>% 
  wrangling_DC_2()
...
survey.picoxystrobin.2.host <- survey.picoxystrobin.2 %>% 
  filter(!source=="Baseline")
...
```{r}
test_source(survey.boscalid.2)

```
Ana by Field
```{r}
test_field (survey.boscalid.2)
```
 
 
 Ana Stats, by host
```{r}
test_host(survey.boscalid.2.host
            )
...
####
 ANOVA.boscalid <- aov(EC50DC ~ Field_Year, survey.boscalid.2)
...
generate_label_df_boscalid <- function(HSD, flev){
  ...
  boxplot.df <- ddply(survey.boscalid.2, flev, function (x) max(fivenum(x$EC50DC)) + 0.1)
...
ggplot(survey.boscalid.2, aes( x = Field_Year, y = EC50DC)) + geom_violin() +
...
```{r}
test_source(survey.tetraconazole.2)
```
 Ana by Field
```{r}
test_field (survey.tetraconazole.2)
```
 
 Ana Stats, by host
```{r}
test_host(survey.tetraconazole.2.host
            )
```
the graph
```{r}
...
```{r}
test_source(survey.picoxystrobin.2)
```

Ana by Field
```{r}
test_field (survey.picoxystrobin.2)
```

 Ana Stats, by host
```{r}
test_host(survey.picoxystrobin.2.host
            )
...
survey.boscalid.2 %>% 
  ...
  bind_rows(survey.tetraconazole.2) %>% 
  bind_rows(survey.picoxystrobin.2) %>% 
select(-c(EC50DC)) %>% 
    unique() %>% 
group_by(source, Host, country ) %>%
  summarize(N= n()) %>%
    ungroup() %>%
        add_row(source = "Total", N  = sum(.$N))

myplot_model_comparison(survey.boscalid.2) + stat_compare_means( method = "anova", paired = TRUE,  aes(label = ..p.signif..), 
                                                                 label.x = 1.5, label.y = 0.28)
```
#Getting the less sensive isolates from boscalid and picoxystrobin
```{r}
boscalid.less.sensive <-survey.boscalid.2 %>% arrange(desc(EC50DC))%>% select(ID) %>% pull()
boscalid.less.sensive.2 <- boscalid.less.sensive[1:10]


picoxystrobin.less.sensive <-survey.picoxystrobin.2 %>% arrange(desc(EC50DC))%>% select(ID) %>% pull()
