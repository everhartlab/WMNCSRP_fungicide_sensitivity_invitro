...
This is an example of fungicide sensitivity test in vitro for Sclerotinia sclerotiorum by discriminatory concetrations approach:

Serial dilution method with 6 concentrations for each fungicide from 22 baseline isolates (never exposed to fungicides) + 22 from between farmer fields and fungicide field trials

EC50 estimated from dose-response curve by ezec-package R (Kamvar, 2016)

Identify a single concentration that can be a discriminatory concentration

Use DC to assess sensitivity of large collection

Data analyses using reproducible methods
