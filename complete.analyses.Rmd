...
#   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
  ...
  #test source
  test_source <- function(filename, vardep, varindep) {
    data <- filename
    vardep <- data$EC50DC
  ...
  test_host <- function(filename, vardep, varindep) {
    data <- filename
    vardep <- data$EC50DC
    varindep <- data$Host
    data1 <-  data %>%
    ...
    ggplot(data = filename,  aes(x = RG, y = logEC50))  +
      ...
      theme(
        ...
        panel.background = element_rect(fill = "white", colour = "grey50")
                 ...
                 "Relative growth (%)", y = expression(bold(Log  (EC[bold("50")])))) + expand_limits(x = c(15, 65)) + scale_x_continuous(name = waiver(), breaks = c(15, 25, 35, 45, 55, 65)) + scale_y_continuous(breaks = c(-3,-2.75,-2.5,-2.25,-2.0,-1.75,-1.5)) + geom_abline(
                   intercept = 0,
                   slope = 1,
                   color = "black",
                   lty = "dashed",
  ...
  ####
   myplot_model_2 <- function(filename, EC50, Estimate.50DC, Host, source) {
    ...
    Host <- data$Host
    source <- data$source
    ggplot(data =filename,  aes(x = EC50, y = Estimate.50DC)) +  geom_point(aes(
   shape = Host,
   colour = source,
   fill = Host
 ),
...
# }
#  
   myplot_model_comparison <- function(filename, Field_Year, EC50DC) {
     data <- filename
    Field_Year<- data$Field_Year
    EC50DC <- data$EC50DC
    Host <- data$Host
    State <- data$State
    ggplot(data =filename,  aes(x = Field_Year, y = EC50DC))  +
   ...
   labs(x = "Field_Year", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                            (bold(ppm ~ bold(
                                               "a.i."
                                            ))))) + theme(
   ...
   myplot_model_comparison.publication <- function(filename, Field_Year, EC50DC) {
     data <- filename
    Field_Year<- data$Field_Year
    EC50DC <- data$EC50DC
    Host <- data$Host
...
myplot_model_1(boscalid.joined.2.clean) 
...
myplot_model_2(tetraconazole.EC50DC)+ expand_limits(x = c(0.04, 0.25), y = c(0.04, 0.25))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.05, 0.1, 0.15, 0.20, 0.25)) + scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.20, 0.25))
...
######### Linear model of EC50  and EC50DC just to corroborate is a GOOD RELATIONSHIP, check normality and homogeneity of variances
myplot_model_2(picoxystrobin.EC50DC)+ expand_limits(x = c(0.04, 0.25), y = c(0.04, 0.25))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.05, 0.1, 0.15, 0.20, 0.25)) + scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.20, 0.25))
...
survey.boscalid <- bind_rows(survey.boscalid.thomas.3,
                             ...
                             boscalid.EC50.2) %>% select(ID, experiment, EC50DC)
#Have to analyze the controls
...
survey.tetraconazole.thomas.2 <- wrangling_survey(survey.tetraconazole.thomas) %>%
  mutate(EC50DC = exp (tetraconazole.lm [[1]][[1]] + tetraconazole.lm[[1]][[2]] * RG))# exponential is the opposite of log
        #using the model the intercept and the coefficient of the model
##
survey.tetraconazole.USA.2 <- survey.tetraconazole.USA %>% mutate(
    experiment = case_when(
      control == 5.2245 |
        response == 2.1955 |
        response == 2.2385 |
...
survey.tetraconazole.USA.3 <- wrangling_survey(survey.tetraconazole.USA.2) %>%
...
survey.tetraconazole.more.USA.2 <-
  survey.tetraconazole.more.USA %>% mutate(
    experiment = case_when(
      response == 2.2885 |
        response == 2.4825|
        response == 2.4805 |
...
survey.tetraconazole.more.USA.3 <- wrangling_survey(survey.tetraconazole.more.USA.2) %>%
  mutate(EC50DC = exp (tetraconazole.lm [[1]][[1]] + tetraconazole.lm[[1]][[2]] * RG))#
##
...
survey.tetraconazole <- bind_rows(survey.tetraconazole.thomas.2,
                             survey.tetraconazole.USA.3,
                             survey.tetraconazole.more.USA.3,
                             survey.tetraconazole.MX.BZ.2, 
                             tetraconazole.EC50.2) %>% select(ID, experiment, EC50DC)
#Have to analyze the controls
...
survey.picoxystrobin.thomas.2 <- wrangling_survey(survey.picoxystrobin.thomas) %>%
  mutate(EC50DC = exp (picoxystrobin.lm [[1]][[1]] + picoxystrobin.lm[[1]][[2]] * RG))# exponential is the opposite of log
        #using the model the intercept and the coefficient of the model
##
survey.picoxystrobin.USA.2 <- survey.picoxystrobin.USA %>% mutate(
    experiment = case_when(
      control == 7.3565 |
        response == 4.8610 |
        control == 7.6690 |
...
survey.picoxystrobin.USA.3 <- wrangling_survey(survey.picoxystrobin.USA.2) %>%
...
survey.picoxystrobin.more.USA <- survey.picoxystrobin.more.USA %>% mutate(response = (response/10), control = (control/10))
 ...
 survey.picoxystrobin.more.USA.2 <-
  survey.picoxystrobin.more.USA %>% mutate(           
  experiment = case_when(
      response == 2.8865 |
        response == 2.7210|
        response == 3.2805 |
...
survey.picoxystrobin.more.USA.3 <- wrangling_survey(survey.picoxystrobin.more.USA.2) %>%
  mutate(EC50DC = exp (picoxystrobin.lm [[1]][[1]] + picoxystrobin.lm[[1]][[2]] * RG))#
##
...
survey.picoxystrobin <- bind_rows(survey.picoxystrobin.thomas.2,
                             survey.picoxystrobin.USA.3,
                             survey.picoxystrobin.more.USA.3,
                             survey.picoxystrobin.MX.BZ.2, 
                             picoxystrobin.EC50.2) %>% select(ID, experiment, EC50DC)
#Have to analyze the controls
...
```{r}
test_source(survey.boscalid.complete %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() )
...
Ana by Field
```{r}
test_field (survey.boscalid.fields)
```
 
 
 Ana Stats, by host
```{r}
test_host(survey.boscalid.complete.host %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() )
...
the graph
```{r}
# stat.test.boscalid <- survey.boscalid.2 %>% dunn_test(EC50DC ~ Field_Year, p.adjust.method = "bonferroni") %>% add_xy_position(x = "Field_Year") %>% dplyr::filter(p.adj.signif != "ns")
#  
# 
# stat.test.boscalid.2 <- survey.boscalid.2 %>%
#      tukey_hsd (EC50DC ~ Field_Year)%>% add_xy_position(x = "Field_Year") %>% dplyr::filter(p.adj.signif != "ns")
# 
# 
#  amod <- aov(EC50DC ~ Field_Year, survey.boscalid.2)
# 
#  stat.test.boscalid.3 <-  HSD.test(amod,"Field_Year", group=TRUE,console=TRUE,
#          main="Yield of sweetpotato\nDealt with different virus")
# 
# myplot_model_comparison(survey.boscalid.2)+ stat_pvalue_manual(stat.test.boscalid, hide.ns = TRUE) +expand_limits(y = c(0.06, 0.18)) +
#   scale_y_continuous(breaks = c(0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18))
...
####
 ANOVA.boscalid <- aov(EC50DC ~ Field_Year, survey.boscalid.fields.2)
  
 tHSD.boscalid <- TukeyHSD( ANOVA.boscalid, ordered = FALSE, conf.level = 0.95) 
  
#
generate_label_df_boscalid <- function(HSD, flev){
  ...
  boxplot.df <- plyr:: ddply(survey.boscalid.fields.2, flev, function (x) max(fivenum(x$EC50DC)) + 0.01)
...
ggplot(survey.boscalid.fields.2, aes( x = Field_Year, y = EC50DC)) + geom_violin(aes( fill = Country_difference)) + scale_fill_manual(values=c( "gray", "white"))+
    geom_text(data = generate_label_df_boscalid(tHSD.boscalid, 'Field_Year'), aes(x = plot.labels, y = V1, label = labels)) +
    labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                              (bold(ppm ~ bold(
                                                  ...
                                                  axis.text.x = element_text(
                                                      ...
                                                      size = 8,
                                                      family = "Arial",
                                                      angle = 30), axis.text.y = element_text(
                                                      face = "bold",
                                                      size = 8,
                                                      family = "Arial"    ),
                                                  panel.background = element_rect(fill = "white", colour = "grey50")) +expand_limits(y = c(0.06, 0.18)) +
  scale_y_continuous(breaks = c(0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18)) + stat_summary(
      fun.y = mean,
      geom = "point",
      shape = 23,
      size = 2,
...
###
# generate_label_df_2 <- function(HSD, flev){
#   # ANOVA <- aov(EC50DC ~ Field_Year, survey.boscalid.2)
...
#   # Get highest quantile for Tukey's 5 number summary and add a bit of space to buffer between    
#   # upper quantile and label placement
#   boxplot.df <- ddply(HSD, flev, function (x) max(fivenum(x$EC50DC)) + 0.1)
#   
#   # Create a data frame out of the factor levels and Tukey's homogenous group letters
#   plot.levels <- data.frame(plot.labels, labels = Tukey.labels[['Letters']],
#                             stringsAsFactors = FALSE)
...
# }
  ...
  phytophthora <-  DunnTest(as.numeric(EC50DC) ~ as.factor(Field_Year), data = survey.boscalid.fields.2, method="bonferroni")
  
  generate_label_df_boscalid_2 <- function(HSD, flev){
    # Extract labels and factor levels from Tukey post-hoc 
    Tukey.levels <- HSD[[1]][,2]
    Tukey.labels <- multcompLetters(Tukey.levels)['Letters']
    plot.labels <- names(Tukey.labels[['Letters']])
    # Get highest quantile for Tukey's 5 number summary and add a bit of space to buffer between    
    # upper quantile and label placement
  ...
  ggplot(survey.boscalid.fields.2, aes( x = Field_Year, y = EC50DC)) + geom_violin(aes( fill = Country_difference)) + scale_fill_manual(values=c( "gray", "white"))+
    geom_text(data = generate_label_df_boscalid_2(phytophthora, 'Field_Year'), aes(x = plot.labels, y = V1, label = labels)) +
    labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                       (bold(ppm ~ bold(
                                         "a.i."
                                       ))))) + theme(
...
```{r}
test_source(survey.tetraconazole.complete  %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() )
```
 Ana by Field
```{r}
test_field (survey.tetraconazole.fields)
```
 
 Ana Stats, by host
```{r}
test_host(survey.tetraconazole.complete.host %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup())
```
the graph
```{r}
# stat.test.tetraconazole <- survey.tetraconazole.2 %>% dunn_test(EC50DC ~ Field_Year, p.adjust.method = "bonferroni") %>% add_xy_position(x = "Field_Year")
# 
# myplot_model_comparison.publication(survey.tetraconazole.2) + stat_pvalue_manual(stat.test.tetraconazole, hide.ns = TRUE,y.position = 1.6,coord.flip = T,step.increase = 0.02,,bracket.shorten = 1) +expand_limits(y = c(0.5, 1.6)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6))
...
mutate(Field_difference = ifelse(
  ...
  mutate(Country_difference = ifelse(
    ...
    "Yes","No"))
####
 ANOVA.tetraconazole <- aov(EC50DC ~ Field_Year, survey.tetraconazole.fields.2)
  
 tHSD.tetraconazole <- TukeyHSD( ANOVA.tetraconazole, ordered = FALSE, conf.level = 0.95) 
  
#
generate_label_df_tetraconazole <- function(HSD, flev){
    # Extract labels and factor levels from Tukey post-hoc 
  Tukey.levels <- HSD[[flev]][,4]
  Tukey.labels <- multcompLetters(Tukey.levels)['Letters']
  plot.labels <- names(Tukey.labels[['Letters']])
  # Get highest quantile for Tukey's 5 number summary and add a bit of space to buffer between    
  # upper quantile and label placement
  boxplot.df <- plyr:: ddply(survey.tetraconazole.fields.2, flev, function (x) max(fivenum(x$EC50DC)) + 0.08)
  # mm <- function(x) return(max(fivenum(x$EC50DC)) + 0.01)
  # boxplot.df <- survey.tetraconazole.2 %>%  group_by(Field_Year) %>% do(mm(.)) 
  # Create a data frame out of the factor levels and Tukey's homogenous group letters
  plot.levels <- data.frame(plot.labels, labels = Tukey.labels[['Letters']],
                            stringsAsFactors = FALSE)
...
ggplot(survey.tetraconazole.fields.2, aes( x = Field_Year, y = EC50DC)) + geom_violin(aes( fill = Country_difference)
  ) + scale_fill_manual(values=c( "gray", "white"))+
    geom_text(data = generate_label_df_tetraconazole(tHSD.tetraconazole, 'Field_Year'), aes(x = plot.labels, y = V1, label = labels)) +
    labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                              (bold(ppm ~ bold(
                                                  "a.i."
                                              ))))) + theme(
                                                  ...
                                                  axis.text.x = element_text(
                                                      ...
                                                      size = 8,
                                                      family = "Arial"    ),
                                                  panel.background = element_rect(fill = "white", colour = "grey50"))+expand_limits(y = c(0.5, 1.6)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6)) + stat_summary(
      fun.y = mean,
      geom = "point",
      shape = 23,
      size = 2,
...
```{r}
test_source(survey.picoxystrobin.complete %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup())
```
...
```
 ...
 Ana Stats, by host
```{r}
test_host(survey.picoxystrobin.complete.host  %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup())
...
mutate(Field_difference = ifelse(
  ...
  mutate(Country_difference = ifelse(
    ...
    "Yes","No"))
####
 ANOVA.picoxystrobin <- aov(EC50DC ~ Field_Year, survey.picoxystrobin.fields.2)
  
 tHSD.picoxystrobin <- TukeyHSD( ANOVA.picoxystrobin, ordered = FALSE, conf.level = 0.95) 
  
#
generate_label_df_picoxystrobin <- function(HSD, flev){
    # Extract labels and factor levels from Tukey post-hoc 
  Tukey.levels <- HSD[[flev]][,4]
  Tukey.labels <- multcompLetters(Tukey.levels)['Letters']
  plot.labels <- names(Tukey.labels[['Letters']])
  # Get highest quantile for Tukey's 5 number summary and add a bit of space to buffer between    
  # upper quantile and label placement
  boxplot.df <- plyr:: ddply(survey.picoxystrobin.fields.2, flev, function (x) max(fivenum(x$EC50DC)) + 0.001)
  # mm <- function(x) return(max(fivenum(x$EC50DC)) + 0.01)
  # boxplot.df <- survey.picoxystrobin.2 %>%  group_by(Field_Year) %>% do(mm(.)) 
  # Create a data frame out of the factor levels and Tukey's homogenous group letters
  plot.levels <- data.frame(plot.labels, labels = Tukey.labels[['Letters']],
                            stringsAsFactors = FALSE)
...
ggplot(survey.picoxystrobin.fields.2, aes( x = Field_Year, y = EC50DC)) + geom_violin(aes( fill = Country_difference  )
) + scale_fill_manual(values=c( "gray", "white"))+
    geom_text(data = generate_label_df_picoxystrobin(tHSD.picoxystrobin, 'Field_Year'), aes(x = plot.labels, y = V1, label = labels)) +
    labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                              (bold(ppm ~ bold(
                                                  "a.i."
                                              ))))) + theme(
                                                  ...
                                                  axis.text.x = element_text(
                                                      ...
                                                      size = 8,
                                                      family = "Arial"    ),
                                                  panel.background = element_rect(fill = "white", colour = "grey50"))+ scale_y_continuous(breaks = c(0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.020, 0.022, 0.024)) + stat_summary(
      fun.y = mean,
      geom = "point",
      shape = 23,
      size = 2,
...
myplot_model_comparison(survey.boscalid.fields.2) + stat_compare_means( method = "anova", paired = TRUE,  aes(label = ..p.signif..), 
                                                                 label.x = 1.5, label.y = 0.28)
```
#Getting the less sensive isolates from boscalid and picoxystrobin
```{r}
boscalid.less.sensive <-survey.boscalid.fields.2 %>% arrange(desc(EC50DC))%>% select(ID) %>% pull()
boscalid.less.sensive.2 <- boscalid.less.sensive[1:10]


picoxystrobin.less.sensive <- survey.picoxystrobin.fields.2 %>% arrange(desc(EC50DC))%>% select(ID) %>% pull()
picoxystrobin.less.sensive.2 <- picoxystrobin.less.sensive[1:10]
# library(kableExtra)
