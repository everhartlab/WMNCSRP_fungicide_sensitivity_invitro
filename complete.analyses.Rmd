...
survey.TM.2 <- survey.TM%>% 
  ...
  select(ID, EC50DC) %>% 
   # is here where now I have to remove the isolates from controls from fields
  dplyr::filter(! ID %in% controls.in.the.plots )
...
```{r}
generate_label( test_source(survey.boscalid.complete %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() ))
...
```
 ...
 Ana Stats, by host
```{r}
generate_label( test_host(survey.boscalid.complete.host %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() ))
...
```{r}
generate_label(test_source(survey.tetraconazole.complete  %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup() ))
```
 Ana by Field
...
```
 ...
 Ana Stats, by host
```{r}
generate_label( test_host(survey.tetraconazole.complete.host %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup()))
```
the graph
...
```{r}
generate_label( test_source(survey.picoxystrobin.complete %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup()))
```
...
```
 ...
 Ana Stats, by host
```{r}
generate_label( test_host(survey.picoxystrobin.complete.host  %>% group_by(ID) %>%
   top_n(1, EC50DC) %>% #keeping the highest value of each ID value
   ungroup()))
