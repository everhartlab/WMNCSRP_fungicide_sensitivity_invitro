...
# }
#  
  generate_label <- function(HSD, flev, surveyy, highlabel){
...
myplot_comparison <- function(filename, Field_Year,EC50DC, highlabel) {
  ...
  ggplot(data = filename,  aes(x = Field_Year, y = EC50DC)) + geom_boxplot() +
    geom_text(data = generate_label(test_field(filename), 'Field_Year', filename,highlabel ),
              aes(x = plot.labels, y = V1, label = labels)) +     labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                                                                                     (bold(ppm ~ bold(
                                                                                                       "a.i."
                                                                                                     ))))) + theme(
...
mutate(Field_difference = ifelse(
  ...
  mutate(Country_difference = ifelse(
    ...
    "Yes","No"))
####
myplot_comparison(survey.tetraconazole.fields.2, highlabel = 0.1)  + expand_limits(y = c(0.5, 1.6)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6)) + geom_hline(yintercept =
...
boscalid.less.sensive.2 <- sort(boscalid.less.sensive[1:10])


picoxystrobin.less.sensive <- survey.picoxystrobin.fields.2 %>% arrange(desc(EC50DC))%>% select(ID) %>% pull()
picoxystrobin.less.sensive.2 <- sort(picoxystrobin.less.sensive[1:10])
# library(kableExtra)
