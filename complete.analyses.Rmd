...
the graph
```{r}
stat.test.boscalid <- survey.boscalid.2 %>% dunn_test(EC50DC ~ Field_Year, p.adjust.method = "bonferroni") %>% add_xy_position(x = "Field_Year")

myplot_model_comparison(survey.boscalid.2)+ stat_pvalue_manual(stat.test.boscalid, hide.ns = TRUE)

  expand_limits(y = c(0.06, 0.18)) +
...
the graph
```{r}
myplot_model_comparison(survey.tetraconazole.2) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6))
```
...
```{r}
myplot_model_comparison(survey.picoxystrobin.2) +  expand_limits(y = c(0.008, 0.024)) + scale_y_continuous(breaks = c(0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.020,0.022,0.024))
...
combination <- (c(boscalid.less.sensive.2,picoxystrobin.less.sensive, genotyping.list))

anyDuplicated(combination)
combination[duplicated(combination)]
```
