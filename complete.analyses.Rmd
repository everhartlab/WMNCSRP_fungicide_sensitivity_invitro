      ...
      theme(
        ...
        panel.background = element_rect(fill = "white", colour = "grey50")
                 ...
                 "Relative growth (%)", y = expression(bold(Log  (EC[bold("50")])))) + geom_abline(
                   ...
                   lty = "dashed",
                   size = 0.3
                  ) + expand_limits(x = c(15, 65)) + scale_x_continuous(name = waiver(), breaks = c(15, 25, 35, 45, 55, 65))  + geom_label(aes(x = 25,
                                                                                                                                                                                        y = -4.30)
                                                                                                                                                                                    ,
                                                                                                                                                                                    label = c(paste  (
                                                                                                                                                                                      " Y =",
...
#+ coord_fixed() #+theme(aspect.ratio=1/5)
#+ coord_equal()
#+ theme(plot.margin = unit(c(-0.5, 1, 1, 1), "cm"))
c.pico
...
#+ theme(plot.margin = unit(c(1, 1, 1, 1), "cm"))#+theme(aspect.ratio=1)
...
#+  theme(plot.margin = unit(c(1, 1, 0, 1), "cm"))#+theme(aspect.ratio=2/3)
                                               
 bb.bos                                              


#+   theme(plot.margin = unit(c(-0.6,1,1,1),"cm"))
cc.pico<- myplot_model_2(picoxystrobin.EC50DC)+ expand_limits(x = c(0.005, 0.018), y = c(0.005, 0.018))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.0075, 0.01, 0.0125, 0.0150, 0.018)) + scale_y_continuous(breaks = c(0.0075, 0.01, 0.0125, 0.0150, 0.018))   + theme(axis.title.y = element_blank(), legend.position = "none", axis.text.y = element_text(
                                                                                                                                                                                                                                angle = 20,
                                                                                                                                       hjust = 1
                                                                                                                                   )) + geom_label(aes(x = 0.0075,
                  y = 0.0148)
              ,
              label = c(paste  (
                " Y =",
              ...
              fontface = "bold")  + labs(tag = "C")  +     theme(  plot.tag = element_text(face = "bold")) 
#+ theme(plot.margin = unit(c(1, 1, -2,1), "cm"))#+theme(aspect.ratio=1/3)
...
####
aa.tet | (bb.bos / cc.pico) + plot_annotation(tag_levels = 'A')

###
bb.bos  + cc.pico + labs(tag = 'A') + labs(tag = 'B')

(bb.bos + labs(tag = 'A')) | ((aa.tet + labs(tag = 'A')) / (cc.pico + labs(tag = 'A')) 
+ 

##
ggarrange(aa.tet,
          ggarrange(bb.bos, cc.pico, nrow = 2,  labels = c( "B", "C")),
          ncol = 2, labels = c("A"))

## ## perfect without margins
ggarrange(aa.tet,
          ggarrange(bb.bos, cc.pico, nrow = 2,align = "v", labels = c( "B", "C") ),
...
aaa.tet <- myplot_comparison(survey.tetraconazole.fields.2, highlabel = 0.1)  + expand_limits(y = c(0.4, 2.5)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6,1.8,2, 2.2, 2.4 ))+ theme(plot.margin = unit(c(1,1,1,1),"cm"))+   theme( axis.title = element_blank(), axis.title.y = element_blank(), legend.position = "none") + theme(plot.margin = unit(c(1,1,1,1),"cm")) + labs(tag = "A")  +     theme(  plot.tag = element_text(face = "bold")) 
aaa.tet

bbb.bos <- myplot_comparison(survey.boscalid.fields.2, highlabel = 0.01) + expand_limits(y = c(0.05, 0.22)) +
  scale_y_continuous(breaks = c(0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.22)) + geom_hline(yintercept =  boscalid.less.sensive$EC50DC[9]
                                                                            , linetype = "dashed")+  theme(plot.margin = unit(c(1,1,1,1),"cm"))+   theme( axis.title.x   = element_blank(), legend.position = "none") +  theme(plot.margin = unit(c(-0.7,1,1,1),"cm")) + labs(tag = "B")  +     theme(  plot.tag = element_text(face = "bold")) # make dynamic the value of intercept, create a new fucntion
bbb.bos


ccc.pico <-
  myplot_comparison(survey.picoxystrobin.fields.2, highlabel = 0.005)  + expand_limits(y = c(0.008, 0.032)) +
  scale_y_continuous(breaks = c(0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.020, 0.022, 0.024, 0.026,0.028,0.03,0.032 )) + geom_hline(yintercept =
                                                                                                              picoxystrobin.less.sensive$EC50DC[10] , linetype = "dashed")  +   theme(plot.margin = unit(c(1, 1, 1, 1), "cm"))  + theme(axis.title.y = element_blank(), legend.position = "none") +   theme(plot.margin = unit(c(-0.7,1,1,1),"cm"))  + labs(tag = "C")  +     theme(  plot.tag = element_text(face = "bold")) 

ccc.pico

#aaa.tet/bbb.bos/ccc.pico + plot_annotation(tag_levels = 'A')
#aaa.tet/bbb.bos/ccc.picoo

# ggarrange(aaa.tet,bbb.bos,ccc.pico, ncol = 1, labels = c("A", "B", "C"), align = "v")
ggarrange(aaa.tet,bbb.bos,ccc.pico, ncol = 1,  align = "v")
# cowplot::plot_grid(aaa.tet, bbb.bos, ccc.pico, align = "v", nrow = 3, rel_widths =   c(1/40, 1/40, 1/40), labels = c("A", "B", "C"))

cowplot::plot_grid(aaa.tet, bbb.bos, ccc.pico, align = "v", nrow = 3, rel_widths =   c(1/40, 1/40, 1/40))
```
