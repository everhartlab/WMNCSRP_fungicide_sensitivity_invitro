...
ggplot(data = filename,  aes(x = RG, y = logEC50))  +
  ...
  size = 3, stroke= 1, colour= "black")  + scale_shape_manual(values = c(21, 24))+ scale_fill_grey(start = 0, end = 1) +
      geom_smooth(
        method = "lm",
        se = FALSE,
        colour = "black",
      ...
      theme(
        ...
        panel.background = element_rect(fill = "white", colour = "grey50")
                 ...
                 "Relative growth (%)", y = expression(bold(Log  (EC[bold("50")])))) + geom_abline(
                   ...
                   lty = "dashed",
                   size = 0.3
                  ) + expand_limits(x = c(15, 65)) + scale_x_continuous(name = waiver(), breaks = c(15, 25, 35, 45, 55, 65))  + geom_label(aes(x = 30,
                                                                                                                                                                                        y = -4.65)
                                                                                                                                                                                    ,
                                                                                                                                                                                    label = c(paste  (
                                                                                                                                                                                      " Y =",
...
myplot_comparison <- function(filename, Field_Year,EC50DC, highlabel) {
  ...
  ggplot(data = filename,  aes(x = Field_Year, y = EC50DC)) + geom_boxplot() +
    geom_text(data = generate_label_2(test_field(filename), 'Field_Year', filename,highlabel),
              aes(x = plot.labels, y = V1, label = labels), size= 4) +     labs(x = "Field", y = expression(bold(EC[bold("50") ~ (bold("D"))]) ~
                                                                                                     (bold(ppm ~ bold(
                                                                                                       "a.i."
                                                                                                     ))))) + theme(
...
myplot_model_1_boscalid(boscalid.joined.2.clean, lm = boscalid.lm) #+ scale_y_continuous(breaks = c(-3,-2.75,-2.5,-2.25,-2.0,-1.75,-1.5))
...
```
# Supplemental graph about baseline
```{r}
survey.tetraconazole.complete %>% filter(source=="Baseline") %>% mutate(Baseline= ifelse(
    ID %in% SI.Screening.Nursery.Field,# if they are in this group
    "White mold screening nursery", "True baseline")) %>%   ggplot(aes(x = Baseline, y = EC50DC,)) + geom_violin() + geom_jitter(aes(shape= Baseline))

myplot_baseline(survey.tetraconazole.complete)
myplot_baseline(survey.boscalid.complete)
myplot_baseline(survey.picoxystrobin.complete)
```

```{r}
#
 # unit(c(t, r, b, l), "cm") with margin sizes at the top, right, bottom, and left, 
#data
a.tet <- myplot_model_1_tetraconazole (tetraconazole.joined.2.clean, lm = tetraconazole.lm) + theme(plot.margin = unit(c(1,1,1,1),"cm"))+   theme(axis.text.x=element_blank(), axis.title = element_blank(), axis.title.y = element_blank(), legend.position = "none") #+ coord_equal()
a.tet

b.bos <- myplot_model_1_boscalid (boscalid.joined.2.clean, lm = boscalid.lm)+  theme(plot.margin = unit(c(-0.5,1,1,1),"cm")) +   theme(axis.text.x=element_blank(), axis.title.x = element_blank(), legend.position = "none") #+ coord_equal()
b.bos

c.pico <- myplot_model_1_picoxystrobin (picoxystrobin.joined.2.clean, lm = picoxystrobin.lm) + theme(plot.margin = unit(c(-0.5,1,1,1),"cm")) +   theme(axis.title.y = element_blank(), legend.position = "none") #+ coord_equal()
c.pico

#Myplot <- multiplot(aa,bb,cc, cols=1)
 # ggarrange(a.tet,b.bos,c.pico, ncol = 1, labels = c("A", "B", "C"))

 #My plot 
 
cowplot::plot_grid(a.tet, b.bos, c.pico, align = "v", nrow = 3, rel_widths =   c(1/40, 1/40, 1/40))
...
aa.tet <-myplot_model_2(tetraconazole.EC50DC)+ expand_limits(x = c(0.27, 1.8), y = c(0.27, 1.8))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.3, 0.6, 0.9, 1.2, 1.5,1.8)) + scale_y_continuous(breaks = c(0.3, 0.6, 0.9, 1.2, 1.5,1.8))+ theme(plot.margin = unit(c(1,1,1,1),"cm"))+   theme( axis.title = element_blank(), axis.title.y = element_blank(), legend.position = "none")+ geom_label(aes(x = 0.45,
                  y = 1.35)
              ,
              label = c(paste  (
                " Y =",
...
bb.bos <-myplot_model_2(boscalid.EC50DC)+ expand_limits(x = c(0.04, 0.25), y = c(0.04, 0.25))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.05, 0.1, 0.15, 0.20, 0.25)) + scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.20, 0.25))+  theme(plot.margin = unit(c(-0.7,1,1,1),"cm")) +   theme( axis.title.x   = element_blank(), legend.position = "none") + geom_label(aes(x = 0.075,
                                                    y = 0.175)
                                                ,
                                                label = c(paste  (
                                                  " Y =",
...
cc.pico<- myplot_model_2(picoxystrobin.EC50DC)+ expand_limits(x = c(0.005, 0.018), y = c(0.005, 0.018))  + scale_x_continuous(name = waiver(),
                                                                             breaks = c(0.0075, 0.01, 0.0125, 0.0150, 0.018)) + scale_y_continuous(breaks = c(0.0075, 0.01, 0.0125, 0.0150, 0.018))  +   theme(plot.margin = unit(c(-0.6,1,1,1),"cm")) + theme(axis.title.y = element_blank(), legend.position = "none") + geom_label(aes(x = 0.003,
                  y = 0.015)
              ,
              label = c(paste  (
                " Y =",
...
#Myplot <- multiplot(aa,bb,cc, cols=1)
#ggarrange(aa.tet,bb.bos,cc.pico, ncol = 1, labels = c("A", "B", "C"))
# Myplot

cowplot::plot_grid(aa.tet, bb.bos, cc.pico, align = "v", nrow = 3, rel_widths =   c(1/40, 1/40, 1/40))
####

aaa.tet <- myplot_comparison(survey.tetraconazole.fields.2, highlabel = 0.1)  + expand_limits(y = c(0.5, 1.6)) + scale_y_continuous(breaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6))+ theme(plot.margin = unit(c(1,1,1,1),"cm"))+   theme( axis.title = element_blank(), axis.title.y = element_blank(), legend.position = "none") 
aaa.tet

bbb.bos <- myplot_comparison(survey.boscalid.fields.2,highlabel = 0.01) + expand_limits(y = c(0.06, 0.18)) +
  scale_y_continuous(breaks = c(0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18)) + geom_hline(yintercept =
                                                                                         0.1606226, linetype = "dashed")+  theme(plot.margin = unit(c(-0.7,1,1,1),"cm"))+   theme( axis.title.x   = element_blank(), legend.position = "none")# make dynamic the value of intercept, create a new fucntion
bbb.bos


ccc.pico <- myplot_comparison(survey.picoxystrobin.fields.2,highlabel = 0.005)  + scale_y_continuous(breaks = c(0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.020, 0.022, 0.024)) + geom_hline(yintercept =
                                                                                                                                                       0.02148932, linetype = "dashed")  +   theme(plot.margin = unit(c(-0.7,1,1,1),"cm")) + theme(axis.title.y = element_blank(), legend.position = "none")

ccc.pico

 ggarrange(aaa.tet,bbb.bos,ccc.pico, ncol = 1, labels = c("A", "B", "C"))
