...
whitemold.inventory <- read_csv("whitemold.inventory.csv")
...
for (variable in unique(whitemold.inventory$Field_Names)) {
    assign(variable, whitemold.inventory %>% filter (Field_Names == variable)%>% pull(ID), envir = .GlobalEnv)
}
```
...
##wrangling survey to get the EC50DC
  ...
  ###Probbaly I wil have to change the specific RG0.02 for RG in the previous functions
...
# }
#  
   myplot_model_comparison <- function(filename, Year_Field, EC50DC) {
     data <- filename
    Year_Field<- data$Year_Field
    EC50DC <- data$EC50DC
    Host <- data$Host
    State <- data$State
    ggplot(data =filename,  aes(x = Year_Field, y = EC50DCC))  +
...
summary(tetraconazole.lm <- lineal_model("logEC50","RG_2",data =  tetraconazole.joined.2.clean))
   # Getting the EC50DC according to the model
tetraconazole.EC50DC <- tetraconazole.joined.2.clean %>% mutate(Estimate.50DC =
                                     #using the model the intercept and the coefficient of the model
                                    exp(tetraconazole.lm[[1]][[1]] + tetraconazole.lm[[1]][[2]] * RG_2)) %>%# here I need to do this dynamic
  select(ID, Estimate.50DC, EC50)  %>%  wrangling_DC()
...
summary(picoxystrobin.lm <- lineal_model("logEC50","RG_0.01",data =  picoxystrobin.joined.2.clean))
   # Getting the EC50DC according to the model
picoxystrobin.EC50DC <- picoxystrobin.joined.2.clean %>% mutate(Estimate.50DC =
                                     #using the model the intercept and the coefficient of the model
                                    exp(picoxystrobin.lm[[1]][[1]] + picoxystrobin.lm[[1]][[2]] * RG_0.01)) %>%# here I need to do this dynamic
  select(ID, Estimate.50DC, EC50)  %>%  wrangling_DC()
...
#wranglind_DC_2
survey.boscalid.2 <- survey.boscalid%>% select(ID, EC50DC) %>% 
group_by(ID) %>%
...
#wranglind_DC_2
survey.tetraconazole.2 <- survey.tetraconazole%>% select(ID, EC50DC) %>% 
group_by(ID) %>%
  top_n(1, EC50DC) %>% #keeping the highest value of each ID value
  ungroup() %>% 
  wrangling_DC()         
...
#wranglind_DC_2
survey.picoxystrobin.2 <- survey.picoxystrobin%>% select(ID, EC50DC) %>% 
group_by(ID) %>%
  top_n(1, EC50DC) %>% #keeping the highest value of each ID value
  ungroup() %>% 
  wrangling_DC()         
