...
```
# New function to detect outliers
```{r}
...
data_for_plots_bos <- boscalid_complete %>% 
  ...
  mutate(Origin = ifelse(
    ...
    ifelse(
      ...
      "Farmer fields","Fungicide field trials"
    )))
...
summary_field_original_boscalid <- WM %>% 
...
unique(data_for_plots_bos2$ID) #49 IDs
...
field_ID_data_boscalid <- data_for_plots_bos2 %>% 
  ...
  select(colnames(data_for_plots_bos2), Field, Year, State) %>%
    left_join (summary_field_original_boscalid) %>% 
  filter(molecule.s._current_season == "boscalid")
  

#transforming to numeric
field_ID_data_boscalid$ID <- as.numeric(field_ID_data_boscalid$ID ) 

#ordering ID
field_ID_data_boscalid <- field_ID_data_boscalid %>% 
arrange(ID)
#creating the table in pdf
field_ID_data_boscalid %>% 
...
summary_field_original_picoxystrobin <- WM %>% 
...
unique(data_for_plots_pico2$ID) #49 IDs

data_for_plots_pico2$ID <- as.factor(data_for_plots_pico2$ID)

field_ID_data_picoxystrobin <- data_for_plots_pico2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
  select(colnames(data_for_plots_pico2), Field, Year, State) %>% 
  left_join(summary_field_original_picoxystrobin)  
  

#transforming to numeric
field_ID_data_picoxystrobin$ID <- as.numeric(field_ID_data_picoxystrobin$ID ) 

#ordering ID
field_ID_data_picoxystrobin <- field_ID_data_picoxystrobin %>% 
arrange(ID)
#creating the table in pdf
field_ID_data_picoxystrobin %>% 
...
summary_field_original_tetraconazole <- WM %>% 
  select(Field, Collection_ID) %>% 
  unique() %>% 
  group_by(Field) %>% 
  summarize(N_isolates = n()) %>% 
...
unique(data_for_plots_tetra2$ID) #49 IDs

data_for_plots_tetra2$ID <- as.factor(data_for_plots_tetra2$ID)

field_ID_data_tetraconazole <- data_for_plots_tetra2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
  select(colnames(data_for_plots_tetra2), Field, Year, State) %>% 
  left_join(summary_field_original_tetraconazole)  
  

#transforming to numeric
field_ID_data_tetraconazole$ID <- as.numeric(field_ID_data_tetraconazole$ID ) 

#ordering ID
field_ID_data_tetraconazole <- field_ID_data_tetraconazole %>% 
arrange(ID)
#creating the table in pdf
field_ID_data_tetraconazole %>% 
...
```{r}
# TM                                                               
data_for_plots_thio2<- data_for_plots_TM%>% 
group_by(Origin)%>%
mutate(Estimate_50DC_outlier = list(get_range2(Estimate.50DC)))%>%
unnest() %>% 
filter(Estimate.50DC >=  outlier) %>% ungroup() %>%  
...
unique(data_for_plots_thio2$ID) #49 IDs

data_for_plots_thio2$ID <- as.factor(data_for_plots_thio2$ID)

field_ID_data_TM <- data_for_plots_thio2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
  select(colnames(data_for_plots_thio2), Field, Year, State) %>% 
  left_join(summary_field_original_TM)  
  

#transforming to numeric
field_ID_data_TM$ID <- as.numeric(field_ID_data_TM$ID ) 
...
#%>% 
 ...
 #Transforming ID in character
  data_for_plots_bos2$ID <- as.character(data_for_plots_bos2$ID)
  #Replacing from form ID to Inventory ID
  data_for_plots_bos2$ID[data_for_plots_bos2$ID == "H-01"]  <- "2220"
    data_for_plots_bos2$ID[data_for_plots_bos2$ID == "62-02"] <- "2098"
data_for_plots_bos2$ID[data_for_plots_bos2$ID == "78-02"] <- "2140"
data_for_plots_bos2$ID[data_for_plots_bos2$ID == "78-05"]  <- "2143"
  data_for_plots_bos2$ID[data_for_plots_bos2$ID == "65B"]  <- "2528"
  data_for_plots_bos2$ID[data_for_plots_bos2$ID =="413"]  <- "2386"
 #Transforming ID in numeric  
data_for_plots_bos2$ID <- as.numeric(data_for_plots_bos2$ID) 
...
data_for_plots_bos_2 <- boscalid_complete %>% 
  ...
  mutate(group = ifelse(
    ID %in% bean_isolates_exposed_to_fungicides,
    "Dry bean isolates","Soybean isolates"
    ))
