...
##NORMALITY TEST-Shapiro_test, NO NORMALS##
...
```
## Filtering by each group to know the medians of each group at first sight
```{r}
...
RG <-
  boscalid_filtered %>% group_by(ID, dose) %>% summarise(median_response = median(response, na.rm = TRUE)) %>%
  spread(dose, median_response) %>%
...
```
#BOSCALID WITH SURVEY(PRE-TEST FUNGICIDE)
```{r}
...
boscalid_filtered_ <- boscalid_filtered_survey %>% 
  group_by(ID) %>% 
  summarise(median_response =median(response, na.rm = TRUE), median_control =median(control, na.rm = TRUE)) %>% 
  mutate(RG= (median_response/median_control)*100) %>% 
...
#boscalid_filtered2 <- boscalid_filtered %>% select(-c(median_response,median_control))
...
RG <-
  picoxystrobin_filtered %>% group_by(ID, dose) %>% summarise(median_response = median(response, na.rm = TRUE)) %>% spread(dose, median_response) %>%
...
#PICOXYSTROBIN WITH SURVEY(PRE-TEST FUNGICIDE)
...
RG <-
  tetraconazole_filtered %>% 
   group_by(ID, dose) %>%
    summarise(median_response = median(response, na.rm = TRUE)) %>% 
     spread(dose, median_response) %>%
...
```
#TETRACONAZOLE WITH SURVEY(PRE-TEST FUNGICIDE)
...
# RG <- TM_filtered %>% group_by(ID,dose) %>% summarise(median_response=median(response,na.rm=TRUE)) %>% spread(dose,median_response) %>%  mutate(RG0.75=((`0.75`/`0`)*100)) %>% mutate(RG1=((`1`/`0`)*100)) %>% mutate(RG1.5=((`1.5`/`0`)*100)) %>% mutate(RG2=((`2`/`0`)*100)) %>% mutate(RG2.5=((`2.5`/`0`)*100)) %>%  mutate(RG10=((`10`/`0`)*100))
# 
# 
# #Ordering the table
# TM_xx <-  xx %>% 
...
TM_data <- TM_data %>% 
  ...
  summarise(median_response =median(response, na.rm = TRUE), median_control =median(control, na.rm = TRUE)) %>% 
  mutate(RG= (median_response/median_control)*100)  %>% 
mutate(resistance = ifelse(RG >24, "resistant", "nonresistant")) 
...
#   group_by(ID) %>% 
#   filter(dose == 0) %>% 
#   summarise(median_response_control =median(response, na.rm = TRUE))
# 
# boscalid_trt <- boscalid_filtered %>% 
#   group_by(ID) %>% 
#   filter(dose != 0) %>% 
#   summarise(median_response_trt =median(response, na.rm = TRUE))
# 
# boscalid_new_dat <- boscalid_control %>% 
#   inner_join(boscalid_trt) %>% 
#   mutate(RG= (median_response_trt/median_response_control)*100) %>% 
#   mutate(Estimate.50DC = exp (-4.407938 + (0.056624*RG)))
# ##
