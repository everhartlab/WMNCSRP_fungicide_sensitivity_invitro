...
Farmer_fields2 <- c(136:442, 455, 456, 466, 468, 470, 471, 478:554, 602:610, 612:635, 671, 672, 682:690, 695:797, 810:823, 834, 835, 848:1024, 1255: 1326, 1491: 1500, 1661:1670, 1831: 2246, 2303: 2342,2362: 2381, 2393: 2573)

baseline_isolates2 <- c(1: 136, 444: 454, 457: 465, 467, 469, 472:477, 555:601, 611, 636:670, 673: 680, 691:694 , 798: 809,  824: 833, 836: 847)
...
boscalid_filtered  <- boscaliddata %>%
  group_by(ID, dose) %>%
  mutate(growth_range = list(get_range(growth))) %>%# a function that i create before
  unnest() %>%
...
#dev.off()
##taking out outliers from Q-Q PLOT $ LEVERAGE TEST, ISOLATES # 1032 from treatment, H-03 & H-04 from survey##
...
###
alga <-  boscalid_filtered %>% filter(dose== 0.2 | dose== 0) %>%  spread(dose, response) %>% rename(control= '0',  response = '0.2') %>% select(ID, repeats, experimental_replicate,  response,control) %>% 
  mutate(ID= as.character(ID))
...
baseline_corroboration <-  boscalid_filtered_ %>% filter(ID %in% baseline_isolates_corroboration)
...
###
protozoa <-  picoxystrobin_filtered %>% filter(dose== 0.01 | dose== 0) %>%  spread(dose, response) %>% rename(control= '0',  response = '0.01') %>% select(ID, repeats, experimental_replicate,  response,control ) %>% 
  mutate(ID= as.character(ID))


# That was using the EC50D of the serial dilution but Iam going to use Real EC50 of those
 
 # picoxystrobin_data <- picoxystrobin_data %>% full_join (protozoa)
...
summary(baseline$Estimate.50DC)
hist((baseline$Estimate.50DC))
baseline_corroboration <-  picoxystrobin_filtered_ %>% filter(ID %in% baseline_isolates_corroboration)
summary(baseline_corroboration$Estimate.50DC)

survey <- final_picoxystrobin_DC %>% filter(ID %in% survey_isolates)
summary(survey$Estimate.50DC)
survey_corroboration <-  picoxystrobin_filtered_ %>% filter(ID %in% survey_isolates_corroboration)
summary(survey_corroboration$Estimate.50DC)
...
ggplot(final_tetraconazole_DC_2,
       ...
       #  + labs(title = "Relationship between EC50 values of tetraconazole using the model y = -1.650985 + 0.044510x", x =
       ...
       # theme(axis.text.x = element_text(size = 12)) + theme(axis.text.y = element_text(size = 12)) + theme (axis.line = element_line(colour = "grey50", size = 1)) + geom_point(shape = 21) + geom_smooth(method = "lm")
       # #dev.off()
...
###
amibae <-  tetraconazole_filtered %>% filter(dose== 2 | dose== 0) %>%  spread(dose, response) %>% rename(control= '0',  response = '2') %>% select(ID, repeats, experimental_replicate,  response,control ) %>% 
    mutate(ID = as.character(ID))
...
## That was using the EC50D of the serial dilution but Iam going to use Real EC50 of those
 ...
 # tetraconazole_data <- tetraconazole_data %>% full_join (amibae)
...
summary(baseline$Estimate.50DC)
hist((baseline$Estimate.50DC))
baseline_corroboration <-  tetraconazole_filtered_ %>% filter(ID %in% baseline_isolates_corroboration)
summary(baseline_corroboration$Estimate.50DC)

survey <- final_tetraconazole_DC %>% filter(ID %in% survey_isolates)
summary(survey$Estimate.50DC)
survey_corroboration <-  tetraconazole_filtered_ %>% filter(ID %in% survey_isolates_corroboration)
summary(survey_corroboration$Estimate.50DC)
...
###WI###
soybean_treatment_WI_year2015 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2015_WI)
#subset(tetraconazole_filtered, ID == "2383", select = c(ID, Estimate.50DC))
#summary(soybean_treatment_WI_year2015$Estimate.50DC)


soybean_treatmentyear_WI_2017 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2017_WI)
#subset(tetraconazole_filtered, ID == "1541", select = c(ID, Estimate.50DC))
#summary(soybean_treatmentyear_WI_2017$Estimate.50DC)

###IA###

soybean_treatment_IA_year2016 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2016_IA)
summary(soybean_treatment_IA_year2016$Estimate.50DC)

soybean_treatment_IA_year2017 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2017_IA)
#subset(tetraconazole_filtered, ID == "1327", select = c(ID, Estimate.50DC))
#summary(soybean_treatment_IA_year2017$Estimate.50DC)

###MI###

soybean_treatment_MI_year2016 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2016_MI)
#subset(tetraconazole_filtered, ID == "1175", select = c(ID, Estimate.50DC))
#summary(soybean_treatment_MI_year2016$Estimate.50DC)

soybean_treatmentyear_MI_2017 <-  tetraconazole_filtered_ %>% filter(ID %in% soybean_treatmentyear2017_MI)
...
TM_zz <-  zz %>%
select(-Estimate.10,-SE.10,-Estimate.90,-SE.90)
names(TM_zz)[names(TM_zz) == "sample"] <- "ID"
 final_TM<-left_join (RG, TM_zz) %>% mutate(logEC50=(log(Estimate.50))) %>% 
    rename(EC50 = Estimate.50)
...
final_TM_DD <- final_TM %>% mutate(Estimate.50DD = exp(1.282440 - (0.004732*RG10)))
# Linear model of EC50  and EC50DD, check normality and homogeneity of variances
final_TM_DD_10 <- lm (Estimate.50DD ~ EC50, final_TM_DD)
...
ggplot(final_TM_DD_10, aes(x = EC50, y = Estimate.50DD)) +  theme(plot.title = element_text(size = 16, face = "bold", hjust = 1)) + labs(title = "Linear regression of thiophanate methyl 10 ppm", x ="EC50 (ppm)", y = "EC50DD (ppm)" ) + geom_point() + geom_smooth(method = "lm")
...
## Discarding all experimental replicate 1 due not consistence in results## anyways it was not consistence in those ones
TM_filtered1 <- TM_filtered %>%
  group_by (ID, dose, repeats, response) %>%
  filter(experimental_replicate==2 | experimental_replicate==3)

#Boxplot by experimental replicate,  

ggplot(TM_filtered1,aes(x=dose, y=response)) + xlim(0, 0.06)  +  geom_boxplot ( aes(group = experimental_replicate), notch = TRUE,  outlier.colour = "blue1",  outlier.shape = 3, outlier.size=2) + xlab("TM dose (ppm)") +
  ylab("response (cm)") + facet_wrap(~  ID ) + ggtitle("Boxplot_TM_by_experimental_replicate_&ID_group_by_ID") + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(breaks = c(1:7)) + geom_point(aes(dose,response, size=1), position = "jitter", size= 1) 
 
##NORMALITY TEST-Shapiro_test, NO NORMALITY##

shapiro.test_TM1 <- TM_filtered1 %>% 
  group_by(ID) %>% 
  do(tidy(shapiro.test(.$response))) 

TMfinal1 <- left_join (shapiro.test_TM1, TM_filtered1) %>% 
  mutate(normality = ifelse(p.value >0.05, "normal", "nonormal")) %>% 
  spread(experimental_replicate, response)
View(TMfinal1)
...
#View(wilcox_TM)
#Getting EC50 by ECtable
zz1 <- EC_table(TM_filtered1, form = response ~ dose)


names(zz1)[names(zz1) == "sample"] <- "ID"
TM_zz1 <-
  zz1 %>% select(c("ID", "Estimate.50")) %>% rename(EC50 = Estimate.50) %>% group_by(ID, EC50) %>% mutate(group = ifelse(
    ID %in% baseline_isolates,
    "baseline",
    ifelse(
    ID %in% survey_isolates,
    ...
    )) %>% ungroup() %>% mutate(fungicide = "TM")
    
TM_zz1$group <- as_factor(TM_zz1$group)
levels(TM_zz1$group)[levels(TM_zz1$group) == "survey_isolates"] <-"Farmer fields"
levels(TM_zz1$group)[levels(TM_zz1$group) == "treatmentyear2016_isolates"] <-"Trial fields"
levels(TM_zz1$group)[levels(TM_zz1$group) == "baseline"] <-"Baseline"

#  Filtering each group
baseline <-  TM_zz1 %>% filter(ID %in% baseline_isolates)
summary(baseline)
survey <-  TM_zz1 %>% filter(ID %in% survey_isolates)
summary(survey)
treatmentyear2016 <- TM_zz1 %>% filter(ID %in% treatmentyear2016_isolates)
summary(treatmentyear2016)
...
#Procedure to get Discriminatory conecntration
 ...
 RG <- TM_filtered1 %>% group_by(ID,dose) %>% summarise(mean_response=mean(response,na.rm=TRUE)) %>% spread(dose,mean_response) %>%  mutate(RG0.75=((`0.75`/`0`)*100)) %>% mutate(RG1=((`1`/`0`)*100)) %>% mutate(RG1.5=((`1.5`/`0`)*100)) %>% mutate(RG2=((`2`/`0`)*100)) %>% mutate(RG2.5=((`2.5`/`0`)*100)) %>% 
...
#select(-Estimate.10,-SE.10,-Estimate.90,-SE.90)
 ...
 # Linear model of log EC50 and relative growth at 0.75ppm, check normality and homogeneity of variances
 finalRG0.75_1 <- lm(logEC50~RG0.75,final_TM1)
 summary(finalRG0.75_1)
# check_assumptions(finalRG0.75)
    # Linear Regression graph oflog EC50 and relative growth at 0.75 ppm
# ggplot(finalRG0.75,aes(x=logEC50, y=RG0.75))+  geom_point() + geom_smooth(method = "lm")
# Linear model of log EC50 and relative growth at 1 ppm, check normality and homogeneity of variances
finalRG1_1 <- lm(logEC50~RG1,final_TM1) 
summary(finalRG1_1)
plot(finalRG1_1)
#check_assumptions(finalRG1)
    # Linear Regression graph of log EC50 and relative growth at 1 ppm 
ggplot(finalRG1_1,aes(x= RG1, y= logEC50))+  geom_point() + geom_smooth(method = "lm")

# Linear model of log EC50 and relative growth at 1.5 ppm, check normality and homogeneity of variances
finalRG1.5_1 <- lm(logEC50~RG1.5,final_TM1) 
summary(finalRG1.5_1)
plot(finalRG1.5_1)
#check_assumptions(finalRG1.5)
    # Linear Regression graph of log EC50 and relative growth at 1.5 ppm
ggplot(finalRG1.5_1,aes(x=RG1.5, y=logEC50))+  geom_point() + geom_smooth(method = "lm")


# Linear model of log EC50 and relative growth at 2 ppm, check normality and homogeneity of variances
finalRG2_1 <- lm(logEC50~RG2,final_TM1) 
summary(finalRG2_1)
plot(finalRG2_1)
#check_assumptions(finalRG2)
      # Linear Regression graph of log EC50 and relative growth at 2 ppm
ggplot(finalRG2_1, aes(x = RG2, y = logEC50)) +  geom_point() + geom_smooth(method = "lm")

# Linear model of log EC50 and relative growth at 10 ppm, check normality and homogeneity of variances
finalRG10 <- lm(logEC50 ~ RG10, final_TM1)
summary(finalRG10)
plot(finalRG10)
#check_assumptions(finalRG2.5)
#Linear Regression graph of log EC50 and relative growth at 10 ppm
...
finalRG2.5_1<- lm(logEC50~RG2.5,final_TM1) 
summary(finalRG2.5_1)
plot(finalRG2.5_1)
#check_assumptions(finalRG2.5)
      # Linear Regression graph of log EC50 and relative growth at 2.5 ppm
ggplot(finalRG2.5_1, aes(x = RG2.5, y = logEC50)) +  geom_point() + geom_smooth()
...
# Getting the EC50DD according to the model
final_TM_DD1 <- final_TM1 %>% mutate(Estimate.50DD = exp(-0.3262858 + (0.0248733*RG2.5)))
# Linear model of EC50  and EC50DD, check normality and homogeneity of variances
final_TM_DD_2.5 <- lm (Estimate.50DD ~ EC50, final_TM_DD1)
summary(final_TM_DD_2.5)
plot(final_TM_DD_2.5)
#check_assumptions(final_TM_DD_10)
#pdf("Linear regression of thiophanate methyl 10 ppm.pdf")   
## Linear Regression graph of EC50 and EC50DD
ggplot(final_TM_DD_2.5, aes(x = EC50, y = Estimate.50DD)) +  theme(plot.title = element_text(size = 16, face = "bold", hjust = 1)) + labs(title = "Linear regression of thiophanate methyl 10 ppm", x ="EC50 (ppm)", y = "EC50DD (ppm)" ) + geom_point() + geom_smooth(method = "lm")
...
# Using the function in order to take out the outliers#
TM_data_filtered_survey <- TM_data %>%
  group_by(ID) %>%
  mutate(response_range = list(get_range(response))) %>%
  unnest() %>%
  mutate(control_range = list(get_range(control))) %>%
...
##USind the model
TM_filtered_ <- TM_data_filtered_survey %>%
  group_by(ID) %>% 
  summarise(mean_response =mean(response, na.rm = TRUE), mean_control =mean(control, na.rm = TRUE)) %>% 
  mutate(RG= (mean_response/mean_control)*100) %>% 
  mutate(Estimate.50DC = exp (-0.3262858 + (0.0248733*RG))) %>% 
select(c(ID, Estimate.50DC)) %>% 
  ungroup()
...
#Transforming before in characters
boscalid_xx$ID <- as.character(boscalid_xx$ID)
#Replacing the ID from the inventory ID
 boscalid_xx$ID[ boscalid_xx$ID =="12B"]  <- "12"
 boscalid_xx$ID[ boscalid_xx$ID=="318"]  <- "2390"
 boscalid_xx$ID[ boscalid_xx$ID =="413"]  <- "2386"
 boscalid_xx$ID[ boscalid_xx$ID =="419"]  <- "2385"
  boscalid_xx$ID[ boscalid_xx$ID == "62-01"] <- "2097"  
  boscalid_xx$ID[ boscalid_xx$ID == "62-02"] <- "2098"
   boscalid_xx$ID[ boscalid_xx$ID == "62-03"] <- "2099"
    boscalid_xx$ID[ boscalid_xx$ID == "62-04"] <- "2100"
 boscalid_xx$ID[ boscalid_xx$ID =="74SS1"]  <- "74"

 boscalid_xx$ID[ boscalid_xx$ID == "78-01"] <- "2139"
  boscalid_xx$ID[ boscalid_xx$ID == "78-02"] <- "2140"


 boscalid_xx$ID[ boscalid_xx$ID == "78-05"]  <- "2143"
 
 boscalid_xx$ID[ boscalid_xx$ID == "H-01"]  <- "2220"
 boscalid_xx$ID[ boscalid_xx$ID == "H-03"] <- "2222"
 boscalid_xx$ID[ boscalid_xx$ID == "H-04"] <- "2223"
  boscalid_xx$ID[ boscalid_xx$ID == "I-20"] <- "2362"
     boscalid_xx$ID[ boscalid_xx$ID =="W212"]  <- "2388"

  
  boscalid_xx$ID[ boscalid_xx$ID == "51C"] <- "2449"
 boscalid_xx$ID[ boscalid_xx$ID == "53B"] <- "2536"
boscalid_xx$ID[ boscalid_xx$ID == "54C"] <- "2453"
boscalid_xx$ID[ boscalid_xx$ID == "60A"] <- "2559"

boscalid_xx$ID[ boscalid_xx$ID == "64D"] <- "2518"
 boscalid_xx$ID[ boscalid_xx$ID == "65B"]  <- "2528"
 boscalid_xx$ID[ boscalid_xx$ID == "71B"] <-
"2505"

 
 boscalid_xx$ID[ boscalid_xx$ID == "S-01"] <- "2320"
 ...
 #Transforming before in characters
boscalid_filtered_$ID <- as.character(boscalid_filtered_$ID)
#Replacing the ID from the inventory ID
 boscalid_filtered_$ID[ boscalid_filtered_$ID =="12B"]  <- "12"
 boscalid_filtered_$ID[ boscalid_filtered_$ID=="318"]  <- "2390"
 boscalid_filtered_$ID[ boscalid_filtered_$ID =="413"]  <- "2386"
 boscalid_filtered_$ID[ boscalid_filtered_$ID =="419"]  <- "2385"
  boscalid_filtered_$ID[ boscalid_filtered_$ID == "62-01"] <- "2097"  
  boscalid_filtered_$ID[ boscalid_filtered_$ID == "62-02"] <- "2098"
   boscalid_filtered_$ID[ boscalid_filtered_$ID == "62-03"] <- "2099"
    boscalid_filtered_$ID[ boscalid_filtered_$ID == "62-04"] <- "2100"
 boscalid_filtered_$ID[ boscalid_filtered_$ID =="74SS1"]  <- "74"

 boscalid_filtered_$ID[ boscalid_filtered_$ID == "78-01"] <- "2139"
  boscalid_filtered_$ID[ boscalid_filtered_$ID == "78-02"] <- "2140"


 boscalid_filtered_$ID[ boscalid_filtered_$ID == "78-05"]  <- "2143"
 
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "H-01"]  <- "2220"
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "H-03"] <- "2222"

  boscalid_filtered_$ID[ boscalid_filtered_$ID == "I-20"] <- "2362"
     boscalid_filtered_$ID[ boscalid_filtered_$ID =="W212"]  <- "2388"

  
  boscalid_filtered_$ID[ boscalid_filtered_$ID == "51C"] <- "2449"
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "53B"] <- "2536"
boscalid_filtered_$ID[ boscalid_filtered_$ID == "54C"] <- "2453"
boscalid_filtered_$ID[ boscalid_filtered_$ID == "60A"] <- "2559"

boscalid_filtered_$ID[ boscalid_filtered_$ID == "64D"] <- "2518"
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "65B"]  <- "2528"
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "71B"] <-
"2505"

 
 boscalid_filtered_$ID[ boscalid_filtered_$ID == "S-01"] <- "2320"

 
 ####
 
 picoxystrobin_yy$ID <- as.character(picoxystrobin_yy$ID)
#Replacing the ID from the inventory ID
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID =="12B"]  <- "12"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID=="318"]  <- "2390"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID =="413"]  <- "2386"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID =="419"]  <- "2385"
  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "62-01"] <- "2097"  
  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "62-02"] <- "2098"
   picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "62-03"] <- "2099"
    picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "62-04"] <- "2100"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID =="74SS1"]  <- "74"

 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "78-01"] <- "2139"
  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "78-02"] <- "2140"


 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "78-05"]  <- "2143"
 
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "H-01"]  <- "2220"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "H-03"] <- "2222"
  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "H-04"] <- "2223"

  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "I-20"] <- "2362"
     picoxystrobin_yy$ID[ picoxystrobin_yy$ID =="W212"]  <- "2388"

  
  picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "51C"] <- "2449"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "53B"] <- "2536"
picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "54C"] <- "2453"
picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "60A"] <- "2559"

picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "64D"] <- "2518"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "65B"]  <- "2528"
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "71B"] <-
"2505"

 
 picoxystrobin_yy$ID[ picoxystrobin_yy$ID == "S-01"] <- "2320"

##
  picoxystrobin_filtered_$ID <- as.character(picoxystrobin_filtered_$ID)
 
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID =="12B"]  <- "12"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID=="318"]  <- "2390"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID =="413"]  <- "2386"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID =="419"]  <- "2385"
  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "62-01"] <- "2097"  
  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "62-02"] <- "2098"
   picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "62-03"] <- "2099"
    picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "62-04"] <- "2100"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID =="74SS1"]  <- "74"

 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "78-01"] <- "2139"
  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "78-02"] <- "2140"


 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "78-05"]  <- "2143"
 
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "H-01"]  <- "2220"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "H-03"] <- "2222"
  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "H-04"] <- "2223"

  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "I-20"] <- "2362"
     picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID =="W212"]  <- "2388"

  
  picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "51C"] <- "2449"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "53B"] <- "2536"
picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "54C"] <- "2453"
picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "60A"] <- "2559"

picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "64D"] <- "2518"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "65B"]  <- "2528"
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "71B"] <-
"2505"

 
 picoxystrobin_filtered_$ID[ picoxystrobin_filtered_$ID == "S-01"] <- "2320"
 
 ####
 
 ##########tetraconazole#######

tetraconazole_bb$ID <- as.character(tetraconazole_bb$ID)
#Replacing the ID from the inventory ID
 tetraconazole_bb$ID[ tetraconazole_bb$ID =="12B"]  <- "12"
 tetraconazole_bb$ID[ tetraconazole_bb$ID=="318"]  <- "2390"
 tetraconazole_bb$ID[ tetraconazole_bb$ID =="413"]  <- "2386"
 tetraconazole_bb$ID[ tetraconazole_bb$ID =="419"]  <- "2385"
  tetraconazole_bb$ID[ tetraconazole_bb$ID == "62-01"] <- "2097"  
  tetraconazole_bb$ID[ tetraconazole_bb$ID == "62-02"] <- "2098"
   tetraconazole_bb$ID[ tetraconazole_bb$ID == "62-03"] <- "2099"
    tetraconazole_bb$ID[ tetraconazole_bb$ID == "62-04"] <- "2100"
 tetraconazole_bb$ID[ tetraconazole_bb$ID =="74SS1"]  <- "74"

 tetraconazole_bb$ID[ tetraconazole_bb$ID == "78-01"] <- "2139"
  tetraconazole_bb$ID[ tetraconazole_bb$ID == "78-02"] <- "2140"


 tetraconazole_bb$ID[ tetraconazole_bb$ID == "78-05"]  <- "2143"
 
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "H-01"]  <- "2220"
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "H-03"] <- "2222"
tetraconazole_bb$ID[ tetraconazole_bb$ID == "H-04"] <- "2223"
  tetraconazole_bb$ID[ tetraconazole_bb$ID == "I-20"] <- "2362"
     tetraconazole_bb$ID[ tetraconazole_bb$ID =="W212"]  <- "2388"

  
  tetraconazole_bb$ID[ tetraconazole_bb$ID == "51C"] <- "2449"
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "53B"] <- "2536"
tetraconazole_bb$ID[ tetraconazole_bb$ID == "54C"] <- "2453"
tetraconazole_bb$ID[ tetraconazole_bb$ID == "60A"] <- "2559"

tetraconazole_bb$ID[ tetraconazole_bb$ID == "64D"] <- "2518"
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "65B"]  <- "2528"
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "71B"] <-
"2505"

 
 tetraconazole_bb$ID[ tetraconazole_bb$ID == "S-01"] <- "2320"

#Transformin now to numeric
  tetraconazole_bb$ID <- as.numeric(tetraconazole_bb$ID)

 ########
  
tetraconazole_filtered_$ID <- as.character(tetraconazole_filtered_$ID)
#Replacing the ID from the inventory ID
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID =="12B"]  <- "12"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID=="318"]  <- "2390"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID =="413"]  <- "2386"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID =="419"]  <- "2385"
  tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "62-01"] <- "2097"  
  tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "62-02"] <- "2098"
   tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "62-03"] <- "2099"
    tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "62-04"] <- "2100"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID =="74SS1"]  <- "74"

 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "78-01"] <- "2139"
  tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "78-02"] <- "2140"


 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "78-05"]  <- "2143"
 
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "H-01"]  <- "2220"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "H-03"] <- "2222"
tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "H-04"] <- "2223"
  tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "I-20"] <- "2362"
     tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID =="W212"]  <- "2388"

  
  tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "51C"] <- "2449"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "53B"] <- "2536"
tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "54C"] <- "2453"
tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "60A"] <- "2559"

tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "64D"] <- "2518"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "65B"]  <- "2528"
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "71B"] <-
"2505"

 
 tetraconazole_filtered_$ID[ tetraconazole_filtered_$ID == "S-01"] <- "2320"

#Transformin now to numeric
  tetraconazole_filtered_$ID <- as.numeric(tetraconazole_filtered_$ID)
  
  ##########TM#######

#Transforming before in characters
TM_zz$ID <- as.character(TM_zz$ID)
#Replacing the ID from the inventory ID
 TM_zz$ID[ TM_zz$ID =="12B"]  <- "12"
 TM_zz$ID[ TM_zz$ID=="318"]  <- "2390"
 TM_zz$ID[ TM_zz$ID =="413"]  <- "2386"
 TM_zz$ID[ TM_zz$ID =="419"]  <- "2385"
  TM_zz$ID[ TM_zz$ID == "62-01"] <- "2097"  
  TM_zz$ID[ TM_zz$ID == "62-02"] <- "2098"
   TM_zz$ID[ TM_zz$ID == "62-03"] <- "2099"
    TM_zz$ID[ TM_zz$ID == "62-04"] <- "2100"
   TM_zz$ID[ TM_zz$ID == "78-01"] <- "2139"
  TM_zz$ID[ TM_zz$ID == "78-02"] <- "2140"


 TM_zz$ID[ TM_zz$ID == "78-05"]  <- "2143"
 
 TM_zz$ID[ TM_zz$ID == "H-01"]  <- "2220"
 TM_zz$ID[ TM_zz$ID == "H-03"] <- "2222"
TM_zz$ID[ TM_zz$ID == "H-04"] <- "2223"
  TM_zz$ID[ TM_zz$ID == "I-20"] <- "2362"
     TM_zz$ID[ TM_zz$ID =="W212"]  <- "2388"

  
  TM_zz$ID[ TM_zz$ID == "51C"] <- "2449"
 TM_zz$ID[ TM_zz$ID == "53B"] <- "2536"
TM_zz$ID[ TM_zz$ID == "54C"] <- "2453"
TM_zz$ID[ TM_zz$ID == "60A"] <- "2559"

TM_zz$ID[ TM_zz$ID == "64D"] <- "2518"
 TM_zz$ID[ TM_zz$ID == "65B"]  <- "2528"
 TM_zz$ID[ TM_zz$ID == "71B"] <-
"2505"

 
 TM_zz$ID[ TM_zz$ID == "S-01"] <- "2320"

#Transformin now to numeric
  TM_zz$ID <- as.numeric(TM_zz$ID)

  ##########
  
  #Transforming before in characters
TM_filtered_$ID <- as.character(TM_filtered_$ID)
#Replacing the ID from the inventory ID
 TM_filtered_$ID[ TM_filtered_$ID =="12B"]  <- "12"
 TM_filtered_$ID[ TM_filtered_$ID=="318"]  <- "2390"
 TM_filtered_$ID[ TM_filtered_$ID =="413"]  <- "2386"
 TM_filtered_$ID[ TM_filtered_$ID =="419"]  <- "2385"
  TM_filtered_$ID[ TM_filtered_$ID == "62-01"] <- "2097"  
  TM_filtered_$ID[ TM_filtered_$ID == "62-02"] <- "2098"
   TM_filtered_$ID[ TM_filtered_$ID == "62-03"] <- "2099"
    TM_filtered_$ID[ TM_filtered_$ID == "62-04"] <- "2100"
   TM_filtered_$ID[ TM_filtered_$ID == "78-01"] <- "2139"
  TM_filtered_$ID[ TM_filtered_$ID == "78-02"] <- "2140"


 TM_filtered_$ID[ TM_filtered_$ID == "78-05"]  <- "2143"
 
 TM_filtered_$ID[ TM_filtered_$ID == "H-01"]  <- "2220"
 TM_filtered_$ID[ TM_filtered_$ID == "H-03"] <- "2222"
TM_filtered_$ID[ TM_filtered_$ID == "H-04"] <- "2223"
  TM_filtered_$ID[ TM_filtered_$ID == "I-20"] <- "2362"
     TM_filtered_$ID[ TM_filtered_$ID =="W212"]  <- "2388"

  
  TM_filtered_$ID[ TM_filtered_$ID == "51C"] <- "2449"
 TM_filtered_$ID[ TM_filtered_$ID == "53B"] <- "2536"
TM_filtered_$ID[ TM_filtered_$ID == "54C"] <- "2453"
TM_filtered_$ID[ TM_filtered_$ID == "60A"] <- "2559"

TM_filtered_$ID[ TM_filtered_$ID == "64D"] <- "2518"
 TM_filtered_$ID[ TM_filtered_$ID == "65B"]  <- "2528"
 TM_filtered_$ID[ TM_filtered_$ID == "71B"] <-
"2505"

 
 TM_filtered_$ID[ TM_filtered_$ID == "S-01"] <- "2320"

#Transformin now to numeric
  TM_filtered_$ID <- as.numeric(TM_filtered_$ID)
...
boscalid_filtered_2 <- boscalid_filtered_ %>% 
rename( EC50 =  Estimate.50DC) %>% 
  mutate(ID = as.numeric(ID))
boscalid_complete <-  rbind.data.frame(boscalid_xx2, boscalid_filtered_2)
boscalid_complete <- ungroup(boscalid_complete)
boscalid_complete$ID <- as.numeric(as.character(boscalid_complete$ID) ) 
baby <- boscalid_complete$ID[duplicated(boscalid_complete$ID)]

#now I will take these repeats from the DC test coming

boscalid_filtered_3 <- boscalid_filtered_2 %>% filter(!ID %in% baby )

#Now substitute
boscalid_complete_2 <-  rbind.data.frame(boscalid_xx2, boscalid_filtered_3)
##look for repetition#
 boscalid_complete_2$ID[duplicated(boscalid_complete_2$ID)]

boscalid_complete_3 <- boscalid_complete_2 %>% 
    mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
    ifelse(
      ID %in% Farmer_fields2,
      "FF","FFT"
    )), Host = ifelse(
    ID %in% drybean,
    "Drybean", ifelse(
 ...
 picoxystrobin_yy2 <- picoxystrobin_yy %>% 
  select(ID, EC50)
#No more this
...
picoxystrobin_filtered_2 <- picoxystrobin_filtered_ %>% 
rename( EC50 =  Estimate.50DC) %>% 
  mutate(ID = as.numeric(ID))
picoxystrobin_complete <-  rbind.data.frame(picoxystrobin_yy2, picoxystrobin_filtered_2)
picoxystrobin_complete <- ungroup(picoxystrobin_complete)
picoxystrobin_complete$ID <- as.numeric(as.character(picoxystrobin_complete$ID) ) 
ball <- picoxystrobin_complete$ID[duplicated(picoxystrobin_complete$ID)]

#now I will take these repeats from the DC test coming

picoxystrobin_filtered_3 <- picoxystrobin_filtered_2 %>% filter(!ID %in% ball )

#Now substitute
picoxystrobin_complete_2 <-  rbind.data.frame(picoxystrobin_yy2, picoxystrobin_filtered_3)
##look for repetition#
 picoxystrobin_complete_2$ID[duplicated(picoxystrobin_complete_2$ID)]

picoxystrobin_complete_3 <- picoxystrobin_complete_2 %>% 
    mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
    ifelse(
...
#######TETRACONAZOLE##
 ...
 tetraconazole_bb2 <- tetraconazole_bb %>% 
  select(ID, EC50) 
...
tetraconazole_filtered_2 <- tetraconazole_filtered_ %>% 
rename( EC50 =  Estimate.50DC) %>% 
  mutate(ID = as.numeric(ID))
tetraconazole_complete <-  rbind.data.frame(tetraconazole_bb2, tetraconazole_filtered_2)
tetraconazole_complete <- ungroup(tetraconazole_complete)
tetraconazole_complete$ID <- as.numeric(as.character(tetraconazole_complete$ID) ) 
shoes <- tetraconazole_complete$ID[duplicated(tetraconazole_complete$ID)]

#now I will take these repeats from the DC test coming

tetraconazole_filtered_3 <- tetraconazole_filtered_2 %>% filter(!ID %in% shoes )

#Now substitute
tetraconazole_complete_2 <-  rbind.data.frame(tetraconazole_bb2, tetraconazole_filtered_3)
##look for repetition#
 tetraconazole_complete_2$ID[duplicated(tetraconazole_complete_2$ID)]

 tetraconazole_complete_3 <- tetraconazole_complete_2 %>% 
    mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
    ifelse(
...
##TM#######
 ...
 TM_zz2 <- TM_zz %>% 
  select(ID, Estimate.50) %>% 
   rename(EC50= Estimate.50)
#No more this
# final_TM_DC <- final_TM_DC %>% select(ID, Estimate.50DC)
####WATCH OUT, Iam naming EC50 although for TMe filtered realy is EC50D
TM_filtered_2 <- TM_filtered_ %>% 
rename( EC50 =  Estimate.50DC) %>% 
mutate(ID = as.numeric(ID))

TM_complete <-  rbind.data.frame(TM_zz2, TM_filtered_2)
TM_complete <- ungroup(TM_complete)
TM_complete$ID <- as.numeric(as.character(TM_complete$ID) ) 
orange <- TM_complete$ID[duplicated(TM_complete$ID)]

#now I will take these repeats from the DC test coming

TM_filtered_3 <- TM_filtered_2 %>% filter(!ID %in% orange )

#Now substitute
TM_complete_2 <-  rbind.data.frame(TM_zz2, TM_filtered_3)
##look for repetition#
 TM_complete_2$ID[duplicated(TM_complete_2$ID)]

 TM_complete_3 <- TM_complete_2 %>% 
    mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
    ifelse(
...
shapiro.test_boscal <- boscalid_complete_3 %>%
  group_by(Origin) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_boscal 
# No normals
shapiro.test_boscal_2 <- boscalid_complete_3 %>%
  group_by(Host) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_boscal_2

 #NO NORMALILTY thats is why  krusKal
#by origin

kruskal.test(EC50 ~ Origin, data = boscalid_complete_3)

#There is difference p-value = 0.00001203

DunnTest(EC50 ~ Origin, data = boscalid_complete_3, method = "bonferroni")
#P VALUE 0.00050 *** FF-Baseline
#P VALUE 0.00093 *** FFT-FF


#by hosts

kruskal.test(EC50 ~ Host, data = boscalid_complete_3)
#There is NO difference p-value 0.1529
...
shapiro.test_pico <-picoxystrobin_complete_3 %>%
  group_by(Origin) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_pico 
  ##NO normals
shapiro.test_pico.2 <-picoxystrobin_complete_3 %>%
  group_by(Host) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_pico.2
...
kruskal.test(EC50 ~ Origin, data = picoxystrobin_complete_3)

#There is NO difference p-value = 0.296


#by hosts

kruskal.test(EC50 ~ Host, data = picoxystrobin_complete_3)
#There is NO difference p-value 0.7074
...
shapiro.test_tetra <- tetraconazole_complete_3 %>%
  group_by(Origin) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_tetra 
# No normals
shapiro.test_tetra_2 <- tetraconazole_complete_3 %>%
  group_by(Host) %>%
  do(tidy(shapiro.test(.$EC50)))
shapiro.test_tetra_2

 #NO NORMALILTY thats is why  krusKal
#by origin

kruskal.test(EC50 ~ Origin, data = tetraconazole_complete_3)

#There NO difference p-value = 0.04619

DunnTest(EC50 ~ Origin, data = tetraconazole_complete_3, method = "bonferroni")
#There is difference at 0.1 then is not difference


#by hosts

kruskal.test(EC50 ~ Host, data = tetraconazole_complete_3)
#There is NO difference p-value 0.
...
data_for_plots_BOS$ID <- as.character(data_for_plots_BOS$ID)

xxx<- data_for_plots_BOS %>% 
left_join(WM, by = c(ID = "Collection_ID")) %>% 
rename(Origin = "Origin.x") %>% select(ID, Estimate.50DC, Origin, Host)
   
#NO NORMALILTY thats is why  krusKal


kruskal.test(Estimate.50DC ~ Host, data = xxx)

#There is NO difference p-value =  0.07177
...
data_for_plots_PICO$ID <- as.character(data_for_plots_PICO$ID)


zzz<- data_for_plots_PICO %>% 
left_join(WM, by = c(ID = "Collection_ID")) %>% 
rename(Origin = "Origin.x") %>% select(ID, Estimate.50DC, Origin, Host)
#NO NORMALILTY thats is why  krusKal
#There is NO difference p-value = 0.3658

kruskal.test(Estimate.50DC ~ Host, data = zzz)



#########tetra

data_for_plots_TETRA$ID <- as.character(data_for_plots_TETRA$ID)

aaa<- data_for_plots_TETRA %>% 
left_join(WM, by = c(ID = "Collection_ID")) %>% 
rename(Origin = "Origin.x") %>% select(ID, Estimate.50DC, Origin, Host)
#NO NORMALILTY thats is why  krusKal

#There is NO difference p-value = 0.1462
kruskal.test(Estimate.50DC ~ Origin, data = aaa)
...
plot_bos <- boscalid_complete_3%>%
  ...
  theme_minimal() +
  labs( x = "Origin", y = "EC50D (ppm)") + theme(
...
##
plot_pico <- picoxystrobin_complete_3%>%
  ggplot(aes(x = Origin, y = EC50)) +
  geom_jitter(
    width = .1,
    height = 0,
  ...
  theme_minimal() +
  labs(x = "Origin", y = "EC50D (ppm)") + theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        axis.title = element_text(size = 14, face = "bold", hjust = 0.5),
    axis.text = element_text(
      face = "bold",
      size = 14,
...
plot_pico
####
plot_tetra <- tetraconazole_complete_3%>%
  ggplot(aes(x = Origin, y = EC50)) +
  geom_jitter(
    width = .1,
    height = 0,
...
#Taking out uncertain host isolates
boscalid_complete_4 <- boscalid_complete_3 %>% filter(!ID == "8" & !ID == "129")
plot_bos_2 <- boscalid_complete_4%>%
...
picoxystrobin_complete_4 <- picoxystrobin_complete_3 %>% filter(!ID == "8"&!ID == "129"  )

plot_pico_2 <- picoxystrobin_complete_4%>%
  ggplot(aes(x = Host, y = EC50)) +
  geom_jitter(
    width = .1,
    height = 0,
...
tetraconazole_complete_4 <- tetraconazole_complete_3 %>% filter(!ID == "8"&!ID == "129"  )

plot_tetra_2 <- tetraconazole_complete_4%>%
  ggplot(aes(x = Host, y = EC50)) +
  geom_jitter(
    width = .1,
    height = 0.5,
...
plot_grid(plot_bos, plot_pico, plot_tetra)
plot_grid(plot_bos_2, plot_pico_2, plot_tetra_2)
...
#No more this
# final_boscalid_DC <- final_boscalid_DC %>% select(ID, Estimate.50DC)
####WATCH OUT, Iam naming EC50 although for boscalide filtered really is EC50D
boscalid_filtered_2 <- boscalid_filtered_ %>% 
rename( EC50 =  Estimate.50DC)
boscalid_complete <-  rbind.data.frame(boscalid_xx2, boscalid_filtered_2)
boscalid_complete <- ungroup(boscalid_complete)
...
##
final_TM_DC <- final_TM_DD1 %>% select(ID, Estimate.50DD) %>% rename(Estimate.50DC= Estimate.50DD)# there is a change a name of the column

TM_complete <- rbind.data.frame(final_TM_DC, TM_filtered_)
TM_complete <- ungroup(TM_complete)
...
#Transformin now to numeric
  boscalid_complete$ID <- as.numeric(boscalid_complete$ID)
...
data_for_plots_bos$Origin <- as_factor(data_for_plots_bos$Origin)


##########picoxystrobin#######

#Transforming before in characters
picoxystrobin_complete$ID <- as.character(picoxystrobin_complete$ID)
#Replacing the ID from the inventory ID
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID =="12B"]  <- "12"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID=="318"]  <- "2390"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID =="413"]  <- "2386"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID =="419"]  <- "2385"
  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "62-01"] <- "2097"  
  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "62-02"] <- "2098"
   picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "62-03"] <- "2099"
    picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "62-04"] <- "2100"
 #picoxystrobin_complete[ picoxystrobin_complete$ID =="74SS1"]  <- "74"

 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "78-01"] <- "2139"
  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "78-02"] <- "2140"


 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "78-05"]  <- "2143"
 
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "H-01"]  <- "2220"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "H-03"] <- "2222"
  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "H-04"] <- "2223"

  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "I-20"] <- "2362"
     picoxystrobin_complete$ID[ picoxystrobin_complete$ID =="W212"]  <- "2388"

  
  picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "51C"] <- "2449"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "53B"] <- "2536"
picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "54C"] <- "2453"
picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "60A"] <- "2559"

picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "64D"] <- "2518"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "65B"]  <- "2528"
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "71B"] <-
"2505"

 
 picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "S-01"] <- "2320"

#Transformin now to numeric
  picoxystrobin_complete$ID <- as.numeric(picoxystrobin_complete$ID)
...
data_for_plots_pico$Origin <- as_factor(data_for_plots_pico$Origin)

##########tetraconazole#######

tetraconazole_complete$ID <- as.character(tetraconazole_complete$ID)
#Replacing the ID from the inventory ID
 tetraconazole_complete$ID[ tetraconazole_complete$ID =="12B"]  <- "12"
 tetraconazole_complete$ID[ tetraconazole_complete$ID=="318"]  <- "2390"
 tetraconazole_complete$ID[ tetraconazole_complete$ID =="413"]  <- "2386"
 tetraconazole_complete$ID[ tetraconazole_complete$ID =="419"]  <- "2385"
  tetraconazole_complete$ID[ tetraconazole_complete$ID == "62-01"] <- "2097"  
  tetraconazole_complete$ID[ tetraconazole_complete$ID == "62-02"] <- "2098"
   tetraconazole_complete$ID[ tetraconazole_complete$ID == "62-03"] <- "2099"
    tetraconazole_complete$ID[ tetraconazole_complete$ID == "62-04"] <- "2100"
 #tetraconazole_complete[ tetraconazole_complete$ID =="74SS1"]  <- "74"

 tetraconazole_complete$ID[ tetraconazole_complete$ID == "78-01"] <- "2139"
  tetraconazole_complete$ID[ tetraconazole_complete$ID == "78-02"] <- "2140"


 tetraconazole_complete$ID[ tetraconazole_complete$ID == "78-05"]  <- "2143"
 
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "H-01"]  <- "2220"
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "H-03"] <- "2222"
tetraconazole_complete$ID[ tetraconazole_complete$ID == "H-04"] <- "2223"
  tetraconazole_complete$ID[ tetraconazole_complete$ID == "I-20"] <- "2362"
     tetraconazole_complete$ID[ tetraconazole_complete$ID =="W212"]  <- "2388"

  
  tetraconazole_complete$ID[ tetraconazole_complete$ID == "51C"] <- "2449"
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "53B"] <- "2536"
tetraconazole_complete$ID[ tetraconazole_complete$ID == "54C"] <- "2453"
tetraconazole_complete$ID[ tetraconazole_complete$ID == "60A"] <- "2559"

tetraconazole_complete$ID[ tetraconazole_complete$ID == "64D"] <- "2518"
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "65B"]  <- "2528"
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "71B"] <-
"2505"

 
 tetraconazole_complete$ID[ tetraconazole_complete$ID == "S-01"] <- "2320"

#Transformin now to numeric
  tetraconazole_complete$ID <- as.numeric(tetraconazole_complete$ID)
...
data_for_plots_tetra$Origin <- as_factor(data_for_plots_tetra$Origin)


##########TM#######

#Transforming before in characters
TM_complete$ID <- as.character(TM_complete$ID)
#Replacing the ID from the inventory ID
 TM_complete$ID[ TM_complete$ID =="12B"]  <- "12"
 TM_complete$ID[ TM_complete$ID=="318"]  <- "2390"
 TM_complete$ID[ TM_complete$ID =="413"]  <- "2386"
 TM_complete$ID[ TM_complete$ID =="419"]  <- "2385"
  TM_complete$ID[ TM_complete$ID == "62-01"] <- "2097"  
  TM_complete$ID[ TM_complete$ID == "62-02"] <- "2098"
   TM_complete$ID[ TM_complete$ID == "62-03"] <- "2099"
    TM_complete$ID[ TM_complete$ID == "62-04"] <- "2100"
   TM_complete$ID[ TM_complete$ID == "78-01"] <- "2139"
  TM_complete$ID[ TM_complete$ID == "78-02"] <- "2140"


 TM_complete$ID[ TM_complete$ID == "78-05"]  <- "2143"
 
 TM_complete$ID[ TM_complete$ID == "H-01"]  <- "2220"
 TM_complete$ID[ TM_complete$ID == "H-03"] <- "2222"
TM_complete$ID[ TM_complete$ID == "H-04"] <- "2223"
  TM_complete$ID[ TM_complete$ID == "I-20"] <- "2362"
     TM_complete$ID[ TM_complete$ID =="W212"]  <- "2388"

  
  TM_complete$ID[ TM_complete$ID == "51C"] <- "2449"
 TM_complete$ID[ TM_complete$ID == "53B"] <- "2536"
TM_complete$ID[ TM_complete$ID == "54C"] <- "2453"
TM_complete$ID[ TM_complete$ID == "60A"] <- "2559"

TM_complete$ID[ TM_complete$ID == "64D"] <- "2518"
 TM_complete$ID[ TM_complete$ID == "65B"]  <- "2528"
 TM_complete$ID[ TM_complete$ID == "71B"] <-
"2505"

 
 TM_complete$ID[ TM_complete$ID == "S-01"] <- "2320"

#Transformin now to numeric
  TM_complete$ID <- as.numeric(TM_complete$ID)

 #Ordering

data_for_plots_TM <- TM_complete %>% 
  group_by(ID) %>% 
  mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
...
data_for_plots_TM$Origin <- as_factor(data_for_plots_TM$Origin)
...
```{r}
#Edgar: Look here
WM <- read.csv("data/WM-fixingproductnamesID2573_modifyingforactiveingredient_7.csv", stringsAsFactors = TRUE)
...
glimpse (data_for_plots_bos)
a <- data_for_plots_bos[!duplicated(data_for_plots_bos),]

data_for_plots_bos <- unique(data_for_plots_bos)### Watach OuT!

data_for_plots_bos2<- data_for_plots_bos%>% 
group_by(Origin)%>%
mutate(Estimate_50DC_outlier = list(get_range2(Estimate.50DC)))%>%
unnest() %>% 
filter(Estimate.50DC >=  outlier) %>% ungroup() %>%  
  #filter(Origin == "FFT"|Origin =="FF" ) %>% 
#, group =="Farmer fields")
...
#     ))
# #%>% 
  #mutate(MOA = "Group_7")%>% ungroup() 


 
summary_field_original <- WM %>% 
  select(Field, Collection_ID) %>% 
  unique() %>% 
  group_by(Field) %>% 
...
unique(data_for_plots_bos2$ID) #55 IDs

data_for_plots_bos2$ID <- as.factor(data_for_plots_bos2$ID)

field_ID_data_boscalid <- data_for_plots_bos2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
...
# ordering the 
field_ID_data_boscalid <- field_ID_data_boscalid%>% 
  arrange(ID) 
     
 unique(field_ID_data_boscalid)
anyDuplicated(field_ID_data_boscalid
              $ID)

field_ID_data_boscalid$ID[duplicated(field_ID_data_boscalid$ID)]
#"1029" "1033" "2220" "564"
field_ID_data_boscalid[!duplicated(field_ID_data_boscalid),]
#taking out the repeats
field_ID_data_boscalid <- field_ID_data_boscalid[-c(9, 27, 29,55), ]
## 54 entries

#creating the table in pdf
# field_ID_data_bos %>%
# kable(format = "markdown") %>%    write2pdf("boscalid_outliers.pdf")
  
# Summarizing 1 BY ORIGIN
field_ID_data_bos_summary1 <- field_ID_data_boscalid %>% 
  group_by (Origin) %>% 
...
field_ID_data_bos_summary2 <- field_ID_data_boscalid %>% 
  group_by ( Host) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1))  %>% 
    ungroup() %>% 
              add_row(Host = "Total", N_isol = sum(.$N_isol))
#creating the table in pdf
...
field_ID_data_bos_summary3 <- field_ID_data_boscalid %>% 
  group_by ( State) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
              add_row(State = "Total", N_isol = sum(.$N_isol))
...
field_ID_data_bos_summary4 <- field_ID_data_boscalid %>% 
...
data_for_plots_pico <- unique(data_for_plots_pico)### Watach OuT!
...
unique(data_for_plots_pico2$ID) #58 IDs

data_for_plots_pico2$ID <- as.factor(data_for_plots_pico2$ID)

field_ID_data_pico <- data_for_plots_pico2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x") %>%     select(colnames(data_for_plots_pico2), Field,  Year, State, molecule.s._current_season, Host) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
    arrange(ID) %>% rename(Funcide = molecule.s._current_season) %>%   arrange(ID)  %>% 
  rename(N_iso_perfield =Num_original_isolates) %>%   select(-c(outlier, Estimate.50DC) )
#first transforming ID variable to charater and then to numeric
field_ID_data_pico$ID <- as.numeric(  as.character(field_ID_data_pico$ID))

# ordering the 
field_ID_data_pico <- field_ID_data_pico%>% 
  arrange(ID) 
     
 unique(field_ID_data_pico)
anyDuplicated(field_ID_data_pico
              $ID)

field_ID_data_pico$ID[duplicated(field_ID_data_pico$ID)]
#"645" "800" 
field_ID_data_pico[!duplicated(field_ID_data_pico),]
#taking out the repeats
field_ID_data_pico <- field_ID_data_pico[-c(8, 15), ]
## 58 entries

#creating the table in pdf
# field_ID_data_pico %>%
#   kable(format = "markdown") %>%    write2pdf("picoxystrobin_outliers.pdf")
  
# Summarizing 1 BY ORIGIN
field_ID_data_pico_summary1 <- field_ID_data_pico %>% 
  group_by (Origin) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
              add_row(Origin = "Total", N_isol = sum(.$N_isol))
    #creating the table in pdf
# field_ID_data_pico_summary1 %>% 
#   kable(format = "markdown") %>%    write2pdf("picoxystrobin_outliers_summary_by_origin_.pdf")
     
# Summarizing 2
field_ID_data_pico_summary2 <- field_ID_data_pico %>% 
  group_by ( Host) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
              add_row(Host = "Total", N_isol = sum(.$N_isol))
#creating the table in pdf
# field_ID_data_pico_summary2 %>% 
#   kable(format = "markdown") %>%    write2pdf("picoxystrobin_outliers_summary_by_hosts.pdf")
# Summarizing 3  BY STATE
field_ID_data_pico_summary3 <- field_ID_data_pico %>% 
  group_by ( State) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
data_for_plots_tetra <- unique(data_for_plots_tetra)### Watach OuT!
...
unique(data_for_plots_tetra2$ID) #55 IDs

data_for_plots_tetra2$ID <- as.factor(data_for_plots_tetra2$ID)


field_ID_data_tetra <- data_for_plots_tetra2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x" ) %>%     select(colnames(data_for_plots_tetra2), Field,  Year, State, molecule.s._current_season, Host) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
    arrange(ID) %>% rename(Funcide = molecule.s._current_season) %>% 
  arrange(ID)  %>% 
  rename(N_iso_perfield =Num_original_isolates) %>%   select(-c(outlier, Estimate.50DC) )
#first transforming ID variable to charater and then to numeric
field_ID_data_tetra$ID <- as.numeric(  as.character(field_ID_data_tetra$ID))

# ordering the 
field_ID_data_tetra <- field_ID_data_tetra%>% 
  arrange(ID) 
     
 unique(field_ID_data_tetra)
anyDuplicated(field_ID_data_tetra
              $ID)

field_ID_data_tetra$ID[duplicated(field_ID_data_tetra$ID)]
#"2320" "2385" "87"
field_ID_data_tetra[!duplicated(field_ID_data_tetra),]
#taking out the repeats
field_ID_data_tetra <- field_ID_data_tetra[-c(5, 53, 55, 56), ]
## 55 entries

#creating the table in pdf
# field_ID_data_tetra %>% 
#   kable(format = "markdown") %>%    write2pdf("tetraconazole_outliers.pdf")
  
# Summarizing 1
field_ID_data_tetra_summary1 <- field_ID_data_tetra %>% 
  group_by (Origin) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
               add_row(Origin = "Total", N_isol = sum(.$N_isol))
#creating the table in pdf      
# field_ID_data_tetra_summary1 %>% 
#   kable(format = "markdown") %>%    write2pdf("tetraconazole_outliers_summary_by_origin.pdf")


# Summarizing 2 by host
field_ID_data_tetra_summary2 <- field_ID_data_tetra %>% 
  group_by ( Host) %>% 
             summarize(N_isol = n()
, Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
  ...
  #creating the table in pdf
# field_ID_data_tetra_summary2 %>% 
#   kable(format = "markdown") %>%    write2pdf("tetraconazole_outliers_summary_by_hosts.pdf")


# Summarizing 3  BY STATE
field_ID_data_tetra_summary3 <- field_ID_data_tetra %>% 
  group_by ( State) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
#creating the table in pdf
# field_ID_data_tetra_summary3 %>% 
#   kable(format = "markdown") %>%    write2pdf("tetraconazole_outliers_summary_by_states.pdf")


# Summarizing 4  BY STATE  NE and host soybeans
field_ID_data_tetra_summary4 <- field_ID_data_tetra %>% 
  group_by (Origin, Host,State) %>% 
             summarize(N_isol = n()) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
data_for_plots_TM <- unique(data_for_plots_TM)### Watach OuT!

data_for_plots_TM2<- data_for_plots_TM%>% 
group_by(Origin)%>%
mutate(Estimate_50DC_outlier = list(get_range2(Estimate.50DC)))%>%
unnest() %>% 
filter(Estimate.50DC >=  outlier) %>% ungroup() %>%  
...
unique(data_for_plots_TM2$ID) #55 IDs

data_for_plots_TM2$ID <- as.factor(data_for_plots_TM2$ID)

field_ID_data_TM <- data_for_plots_TM2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x") %>%     select(colnames(data_for_plots_TM2), Field,  Year, State, molecule.s._current_season, Host) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
    arrange(ID) %>% rename(Funcide = molecule.s._current_season) %>% 
  arrange(ID)  %>% 
  rename(N_iso_perfield =Num_original_isolates) %>%   select(-c(outlier, Estimate.50DC) )
#first transforming ID variable to charater and then to numeric
field_ID_data_TM$ID <- as.numeric(  as.character(field_ID_data_TM$ID))

# ordering the 
field_ID_data_TM <- field_ID_data_TM%>% 
  arrange(ID) 
     
 unique(field_ID_data_TM)
anyDuplicated(field_ID_data_TM
              $ID)

field_ID_data_TM$ID[duplicated(field_ID_data_TM$ID)]
#"1029"  "2220" 
field_ID_data_TM[!duplicated(field_ID_data_TM),]
#taking out the repeats
field_ID_data_TM <- field_ID_data_TM[-c(25, 46, 59), ]
## 56 entries

#creating the table in pdf 
# field_ID_data_TM %>%
#   kable(format = "markdown") %>%    write2pdf("TM_outliers.pdf")
  
# Summarizing 1 by Origin
field_ID_data_TM_summary1 <- field_ID_data_TM %>% 
  group_by (Origin) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
              add_row(Origin = "Total", N_isol = sum(.$N_isol))
...
#creating the table in pdf
# field_ID_data_TM_summary1 %>% 
#   kable(format = "markdown") %>%    write2pdf("TM_outliers_summary_by_origin.pdf")

# Summarizing 2 by Host
field_ID_data_TM_summary2 <- field_ID_data_TM %>% 
  group_by ( Host) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
              add_row(Host = "Total", N_isol = sum(.$N_isol))
#creating a table
# field_ID_data_TM_summary2 %>% 
#   kable(format = "markdown") %>%    write2pdf("TM_outliers_summary_by_hosts.pdf")


# Summarizing 3  BY STATE
field_ID_data_TM_summary3 <- field_ID_data_TM %>% 
  group_by ( State) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
#creating the table in pdf
# field_ID_data_TM_summary3 %>% 
#   kable(format = "markdown") %>%    write2pdf("TM_outliers_summary_by_states.pdf")


# Summarizing 4  BY STATE  NE and host soybeans
field_ID_data_TM_summary4 <- field_ID_data_TM %>% 
  group_by (Origin, Host,State) %>% 
             summarize(N_isol = n()) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
```{r}
#BOSACLID doing the table required from Sydney
...
levels(WM$Fungicide_current_season)[levels(WM$Fungicide_current_season) == "No_info"] <- "unknown"

levels(WM$molecule.s._current_season)[levels(WM$molecule.s._current_season) == ""] <- "unknown"

levels(WM$Fungicide_previous_seasons)[levels(WM$Fungicide_previous_seasons) == ""] <- "unknown"


levels(WM$molecule.s._previous_seasons)[levels(WM$molecule.s._previous_seasons) == ""] <- "unknown"


WM$molecule.s._current_season <- as.character(WM$molecule.s._current_season)
WM_ <- WM%>%
  mutate(
  ...
  data_for_plots_bos$ID[duplicated(data_for_plots_bos$ID)]
  #1, 1026, 1027, 1029, 1033, 118, 123, 20, 2098, 21, 2385(2), 2386, 2388, 467,558, 564, 581, 2140, 8, 87, 2220, 2362, NA
  
  data_for_plots_TOTAL <- data_for_plots_bos[-c(1,3,4,5,6,7,8,11,12,14,15,18,20,21,23,2429,31,33,34,35,36,38,134, 229 ), ]
  #tested
  data_for_plots_TOTAL$ID
  ...
  field_ID_data_bos <- data_for_plots_bos2 %>% 
    full_join(WM_, by = c(ID = "Collection_ID")) %>% 
    rename(Origin = "Origin.x") %>%     select(colnames(data_for_plots_bos2), Field,  Year, State, Group_MOA_current_season, Host) %>%
     ungroup() %>%
    rename(Group =Group_MOA_current_season) %>% 
    group_by(Origin, Field,Year, State,Group, Host) %>% 
    mutate (N_isolperfield = n())      %>%  
    ungroup() %>% 
    select(-c(outlier, Estimate.50DC) ) %>% 
        mutate(Origin = ifelse(
    ID %in% baseline_isolates2,
    "Baseline",
    ifelse(
    ...
    select(ID, State, Field, Year, Origin, Group, Host, N_isolperfield)
 # ordering the 
  field_ID_data_bos$ID <- as.numeric( field_ID_data_bos$ID)    field_ID_data_bos <- field_ID_data_bos %>% arrange(ID)
  
  
  #first transforming ID variable to charater and then to numeric
  # field_ID_data_bos$ID <- as.numeric(  as.character(field_ID_data_bos$ID))
  
 
 
       
   unique(field_ID_data_bos)
  anyDuplicated(field_ID_data_bos
                $ID)
  
  data_for_plots_bos2$ID[duplicated(data_for_plots_bos2$ID)]
  #"1029" "1033" "2220" "564"
 data_for_plots_bos2[!duplicated(data_for_plots_bos2),]
  #taking out the repeats
  data_for_plots_bos2 <- data_for_plots_bos2[-c(9, 27, 29,55), ]
  ## 54 entries
  #outliersBOSCALID
   data_for_plots_bos2$ID  #54
  #tested
  data_for_plots_TOTAL$ID #207
...
notoutlierbuttested_bos <-( data_for_plots_TOTAL$ID[! data_for_plots_TOTAL$ID %in% data_for_plots_bos2$ID])

notoutlierbuttested_pico <-( data_for_plots_TOTAL$ID[! data_for_plots_TOTAL$ID %in% data_for_plots_bos2$ID])

notoutlierbuttested_tetra <-( data_for_plots_TOTAL$ID[! data_for_plots_TOTAL$ID %in% data_for_plots_bos2$ID])

notoutlierbuttested_TM <-( data_for_plots_TOTAL$ID[! data_for_plots_TOTAL$ID %in% data_for_plots_bos2$ID])

# fieldSS$ID <- as.numeric(fieldSS$ID)
# fieldSS <- fieldSS %>% arrange(ID)


 

field_ID_data_bos$ID <- as.numeric(field_ID_data_bos$ID)

nottested <- field_ID_data_bos$ID[! field_ID_data_bos$ID %in% data_for_plots_TOTAL$ID]
  
    

FieldSS<- field_ID_data_bos %>%
mutate(bos= case_when(
ID%in% data_for_plots_bos2$ID ~ 1,
ID%in% notoutlierbuttested_bos~ 0,
ID%in% nottested ~NA_real_ , 
TRUE ~ NA_real_),
pico= case_when(
ID%in% field_ID_data_pico$ID ~ 1,
ID%in% notoutlierbuttested_pico ~ 0,
ID%in% nottested ~NA_real_ ,
TRUE ~ NA_real_), 
tetra= case_when(
ID%in% field_ID_data_tetra$ID ~ 1,
ID%in% notoutlierbuttested_tetra ~ 0,
ID%in% nottested ~NA_real_ , 
TRUE ~ NA_real_), 
TM= case_when(
ID%in% field_ID_data_TM$ID ~ 1,
ID%in% notoutlierbuttested_TM ~ 0,
ID%in% nottested ~NA_real_ , 
TRUE ~ NA_real_)) 
 
  hello <- FieldSS %>%   filter(Group== "Group_one"|Group=="Group_3"|Group=="Group_7"|Group=="Group_11")
 

bye <- apply(hello, 1, sum)
#water <- cbind(hello, bye)
fusarium <- water %>% filter(bye>=2) %>% arrange(bye) 
hello2 <- FieldSS %>%  select(bos, pico, tetra, TM)
bye2 <- apply(hello2, 1, sum)
hello3 <- FieldSS 
...
data_for_plots_BOS <- data_for_plots_bos[!duplicated(data_for_plots_bos),]

data_for_plots_bos$ID[duplicated(data_for_plots_bos$ID)]
#1, 1026, 1027, 1029, 1033, 118, 123, 20, 2098, 21, 2385(2), 2386, 2388, 467,558, 564, 581, 2140, 8, 87, 2220, 2362

data_for_plots_BOS <- data_for_plots_bos[-c(1,3,4,5,6,7,8,11,12,14,15,18,20,21,23,24,31,33,34,35,36,38,134 ), ]

data_for_plots_BOS <- data_for_plots_BOS %>% ungroup()
...
# Summarizing 1 by Origin
data_for_plots_summary1 <- data_for_plots_BOS %>% 
  group_by (Origin) %>% 
             summarize(N_isol = n(), Percent = round(100 * n() / nrow(.), digits = 1)) %>% 
             ungroup() %>% 
              add_row(Origin = "Total", N_isol = sum(.$N_isol))
...
#creating the table in pdf
# data_for_plots_summary1 %>%
#   kable(format = "markdown") %>%    write2pdf("summary_by_origin.pdf")
...
data_for_plots_BOS <- # check this out
data_for_plots_bos[!duplicated(data_for_plots_bos$ID),]
##NORMALITY TEST-Shapiro_test

shapiro.test_boscal <- data_for_plots_bos %>%
  group_by(Origin) %>%
  do(tidy(shapiro.test(.$Estimate.50DC)))
shapiro.test_boscal 
...
kruskal.test(Estimate.50DC ~ Origin, data = data_for_plots_bos)

#There is difference p-value = 0.0002899

kruskal.test(Estimate.50DC ~ Origin, data = data_for_plots_BOS)
#There is difference p-value 0.0009201

DunnTest(Estimate.50DC ~ Origin, data = data_for_plots_bos, method = "bonferroni")
#P VALUE 0.0148 * FF-Baseline

DunnTest(Estimate.50DC ~ Origin, data = data_for_plots_BOS, method = "bonferroni")
#FF-FFT            -32.97727 0.0023 **


data_for_plots_BOS %>% group_by(Origin) %>% summarize(mean(Estimate.50DC))
####### pico



data_for_plots_pico$ID[duplicated(data_for_plots_pico$ID)]
#1, 1026, 1027, 1029, 118, 123, 20, 2098, 21, 2385(2), 2386, 2388, 2390, 467, 475, 2100, 645, NA,  2140, 2143, 8, 800,87, 2220, 2222,2223

data_for_plots_PICO <-
data_for_plots_pico[!duplicated(data_for_plots_pico$ID),]
...
kruskal.test(Estimate.50DC ~ Origin, data = data_for_plots_pico)
#There is difference p-value = 0.03845
#There is no difference p-value = 0.06641

kruskal.test(Estimate.50DC ~ Origin, data = data_for_plots_PICO)
#There is no difference p-value = 0.157

# dunnTest(Estimate.50DC ~ Origin, data = data_for_plots_pico, method = "bonferroni")
...
data_for_plots_tetra$ID[duplicated(data_for_plots_tetra$ID)]

#1, 1026, 1027, 1029, 1033,118, 123, 20, 2098, 2385(2),2386(2), 2388,467,581,2098,2139,2140,2143,8,87,2220,2222,2320
# whatis  knew
data_for_plots_TETRA <-  data_for_plots_tetra[!duplicated(data_for_plots_tetra$ID),]

data_for_plots_TETRA$ID[duplicated(data_for_plots_TETRA$ID)]
##NORMALITY TEST-Shapiro_test
shapiro.test_tetra <- 
data_for_plots_TETRA %>%
  group_by(Origin) %>%
  do(tidy(shapiro.test(.$Estimate.50DC)))
shapiro.test_tetra
...
kruskal.test(Estimate.50DC ~ Origin, data = data_for_plots_tetra)

#There is NO difference p-value = 0.2738

#There is NO difference p-value = 0.1537
...
plot_bos <- data_for_plots_bos%>%
  ggplot(aes(x = Origin, y = Estimate.50DC)) +
  geom_jitter(
    width = .1,
    height = 0,
    shape = 21,
...
plot_bos
plot_bos2 <- 
  ggplot(data_for_plots_bos , aes(x = Origin, y = Estimate.50DC))  +  geom_boxplot(notch = TRUE,
                                                                                  outlier.colour = "chartreuse2",
                                                                                  outlier.size = 2)+
  theme_minimal() +
  labs(title = "Boscalid", x = "Origin", y = "EC50D (ppm)") + theme(
    plot.title = element_text(
      size = 10,
      face = "bold",
      hjust = 0.5,
...
plot_pico
plot_pico2 <- 
ggplot(data_for_plots_pico , aes(x = Origin, y = Estimate.50DC))  +  geom_boxplot(notch = TRUE,
                                                                                  outlier.colour = "chocolate1",
                                                                                  outlier.size = 2) +theme_minimal() +
  labs(title = "Picoxystrobin", x = "Origin", y = "EC50D (ppm)") + theme(
    plot.title = element_text(
      size = 10,
      face = "bold",
      hjust = 0.5,
...
plot_tetra
plot_tetra2 <- 
ggplot(data_for_plots_tetra , aes(x = Origin, y = Estimate.50DC))  +  geom_boxplot(notch = TRUE,
                                                                                  outlier.colour = "deepskyblue",
                                                                                  outlier.size = 2) + theme_minimal() +
  labs(title = "Tetraconazole", x = "Origin", y = "EC50D (ppm)") + theme(
    plot.title = element_text(
      size = 10,
      face = "bold",
      hjust = 0.5,
...
plot_TM <- data_for_plots_TM%>%
  ...
  theme_minimal() +
  labs(title = "TM", x = "Origin", y = "EC50D (ppm)") + theme(
    plot.title = element_text(
      size = 16,
      face = "bold",
      hjust = 0.5,
...
plot_TM
plot_TM2 <- 
ggplot(data_for_plots_TM, aes(x = Origin, y = Estimate.50DC))  +  geom_boxplot(notch = TRUE,
                                                                                  outlier.colour = "deepskyblue",
                                                                                  outlier.size = 2) + theme_minimal() +
  labs(title = "TM", x = "Origin", y = "EC50D (ppm)") + theme(
    plot.title = element_text(
...
data_for_plots_bos_2 <- boscalid_complete %>% 
  ...
  mutate(group = ifelse(
    ID %in% bean_isolates_exposed_to_fungicides,
    "Dry bean","Soybean"## check this out
    ))
...
plot_bos_2 <- data_for_plots_bos_2%>%
  ...
  theme_minimal() +
  labs( x = "Group", y = "EC50D (ppm)") + theme(
    panel.border = element_rect(colour = "black", fill=NA, size=1),
        axis.title = element_text(size = 10, face = "bold", hjust = 0.5),
    axis.text = element_text(
      face = "bold",
