...
field_ID_data_bos <- data_for_plots_bos2 %>% 
  ...
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(data_for_plots_bos2), Field,  Year, State, molecule.s._current_season) %>% 
...
field_ID_data_pico <- data_for_plots_pico2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(data_for_plots_pico2), Field,  Year, State, molecule.s._current_season) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
...
field_ID_data_pico_summary3 %>% 
  kable(format = "markdown") %>%    write2pdf("picoxystrobin_outliers_summary_bystates.pdf")
...
field_ID_data_tetra <- data_for_plots_tetra2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(data_for_plots_tetra2), Field,  Year, State, molecule.s._current_season) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
    arrange(ID) %>% rename(Funcide = molecule.s._current_season) %>% 
  arrange(ID)  %>% 
...
#taking out the repeats
field_ID_data_tetra <- field_ID_data_tetra[-c(5, 53, 56), ]
## 56 entries
...
field_ID_data_TM <- data_for_plots_TM2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>%     select(colnames(data_for_plots_TM2), Field,  Year, State, molecule.s._current_season) %>% 
  left_join(summary_field_original)  %>% 
  mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
    arrange(ID) %>% rename(Funcide = molecule.s._current_season) %>% 
  arrange(ID)  %>% 
...
#taking out the repeats
field_ID_data_TM <- field_ID_data_TM[-c(25, 46), ]
## 56 entries
