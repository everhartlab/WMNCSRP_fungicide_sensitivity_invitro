...
field_ID_data <- data_for_plots_bos2 %>% 
  inner_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
  select(colnames(data_for_plots_bos2), Field)
...
#For repeated
data_for_plots_pico2$ID[duplicated(data_for_plots_pico2$ID)]
...
#For repeated
data_for_plots_tetra2$ID[duplicated(data_for_plots_tetra2$ID)]
