...
boscalid_complete$ID[ boscalid_complete$ID == "54C"] <- "2559"
boscalid_complete$ID[ boscalid_complete$ID == "60A"] <- ""
...
picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "54C"] <- "2559"
picoxystrobin_complete$ID[ picoxystrobin_complete$ID == "60A"] <- ""
...
tetraconazole_complete$ID[ tetraconazole_complete$ID == "54C"] <- "2559"
tetraconazole_complete$ID[ tetraconazole_complete$ID == "60A"] <- ""
...
#Replacing the ID from the inventory ID
 ...
 #TM_complete[ TM_complete$ID =="74SS1"]  <- "74"
...
TM_complete$ID[ TM_complete$ID == "54C"] <- "2559"
TM_complete$ID[ TM_complete$ID == "60A"] <- ""
...
combiningfungicides_outliersEC50DC <- rbind(data_for_plots_bos2, data_for_plots_pico2, data_for_plots_tetra2)
#write.csv(combiningfungicides_outliersEC50DC, file = "combiningfungicides_outliersEC50DC2.csv") It must be the same object was saved now with 2 because it reliasing differnet data
 

  combiningfungicides_outliersEC50DC_2 <- combiningfungicides_outliersEC50DC %>% 
   mutate( Origin_Host_Fungicide = paste( Origin, Host, Fungicide)) %>% 
   select(ID, Origin_Host_Fungicide) %>% group_by(Origin_Host_Fungicide) %>% 
   summarize(n=n()) %>% 
   arrange(desc(n))
...
```{r}
#Edgar: Look here
WM <- read.csv("data/WM-fixingproductnamesID2573_modifyingforactiveingredient_6.csv", stringsAsFactors = TRUE)
```
...
unique(data_for_plots_bos2$ID) #46 IDs
...
field_ID_data_bos <- data_for_plots_bos2 %>% 
  ...
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
    select(colnames(data_for_plots_bos2), Field,  Year, State) %>% 
  left_join(summary_field_original)  %>% 
  arrange(ID)  %>% 
  rename(N_isolates_perfield =Num_original_isolates) %>%   select(-outlier)
...
# ordering the 
field_ID_data_bos <- field_ID_data_bos%>% 
  arrange(ID) 
...
field_ID_data_bos$ID[duplicated(field_ID_data_bos$ID)]
#"1029" "1033" "2220" "564"
field_ID_data_bos[!duplicated(field_ID_data_bos),]
#taking out the repeats
field_ID_data_bos <- field_ID_data_bos[-c(9, 27, 29,56), ]
## 55 entries
...
# Summarizing 1
field_ID_data_bos_summary1 <- field_ID_data_bos %>% 
...
# Summarizing 2
field_ID_data_bos_summary2 <- field_ID_data_bos %>% 
  group_by ( Host) %>% 
             summarize(N_isol = n()) %>% 
             ungroup() %>% 
...
field_ID_data_bos_summary3 <- field_ID_data_bos %>% 
  group_by ( State) %>% 
             summarize(N_isol = n()) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
field_ID_data_bos_summary4 <- field_ID_data_bos %>% 
  group_by (Origin, Host,State) %>% 
             summarize(N_isol = n()) %>% 
             ungroup() %>%
              arrange(desc(N_isol)) %>% 
...
#working on data_for_plots_bos for summarize an combining with WM

data_for_plots_bos$ID <- as.factor(data_for_plots_bos$ID)

 x <- data_for_plots_bos %>% inner_join(WM, by = c(ID = "Collection_ID")) %>% rename(Origin = "Origin.x") %>% select(colnames(data_for_plots_bos),   Host, Year, State) %>% arrange(desc(ID)) %>% 
  group_by(Origin, Year, State) %>% summarize(N_isolates= n())
 
 #
 x_ <- (data_for_plots_bos) %>% unique() %>% 
   inner_join(WM, by = c(ID = "Collection_ID")) %>% rename(Origin = "Origin.x") %>% select(colnames(data_for_plots_bos),   Host, Year, State) %>% arrange(desc(ID)) %>% 
  group_by(Host) %>% summarize(N_isolates= n())
 
 x_1 <- (data_for_plots_bos) %>% unique() %>% 
   inner_join(WM, by = c(ID = "Collection_ID")) %>% rename(Origin = "Origin.x") %>% select(colnames(data_for_plots_bos),   Host, Year, State) %>% arrange(desc(ID)) %>% filter(Host=="drybeans") %>% 
  group_by(State) %>% summarize(N_isolates= n()) %>% arrange(desc(N_isolates))
 
 x_2 <- (data_for_plots_bos) %>% unique() %>% 
   inner_join(WM, by = c(ID = "Collection_ID")) %>% rename(Origin = "Origin.x") %>% select(colnames(data_for_plots_bos),   Host, Year, State) %>% arrange(desc(ID)) %>% filter(Host=="soybeans") %>% 
  group_by(State) %>% summarize(N_isolates= n()) %>% arrange(desc(N_isolates))
 
 x_3 <- (data_for_plots_bos) %>% unique() %>% 
   inner_join(WM, by = c(ID = "Collection_ID")) %>% rename(Origin = "Origin.x") %>% select(colnames(data_for_plots_bos),   Host, Year, State) %>% arrange(desc(ID)) %>% filter(Host=="soybeans"|State == "NE")
...
unnest() %>% 
filter(Estimate.50DC >=  outlier) %>% ungroup() %>%  
  #filter(Origin == "Fungicide field trials"|Origin =="Farmer fields" ) %>% 
#, group =="Farmer fields")
  #, group == "Fungicide field trials", group =="Farmer fields" )%>%
ungroup()  %>% 
  #group_by(ID) %>% 
...
unique(data_for_plots_pico2$ID) #46 IDs

data_for_plots_pico2$ID <- as.factor(data_for_plots_pico2$ID)

field_ID_data <- data_for_plots_pico2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
    select(colnames(data_for_plots_pico2), Field,  Year, State) %>% 
  left_join(summary_field_original)  %>% 
  arrange(ID)  %>% 
  rename(N_isolates =Num_original_isolates) %>% 
  select(-outlier) %>% 
  arrange(ID) %>% 
  group_by (Origin, Host) %>% 
             summarize(N_isol = n())
...
#Transforming ID in character
data_for_plots_pico2$ID <- as.character(data_for_plots_pico2$ID)
#Replacing from form ID to Inventory ID
data_for_plots_pico2$ID[data_for_plots_pico2$ID == "H-03"] <- "2222"
data_for_plots_pico2$ID[data_for_plots_pico2$ID == "51C"] <- "2449"
data_for_plots_pico2$ID[data_for_plots_pico2$ID == "53B"] <- "2536"
data_for_plots_pico2$ID[data_for_plots_pico2$ID == "54C"] <- "2453"
data_for_plots_pico2$ID[data_for_plots_pico2$ID == "71B"] <-
"2505"
data_for_plots_pico2$ID[data_for_plots_pico2$ID=="318"]  <- "2390"
data_for_plots_pico2$ID[data_for_plots_pico2$ID =="413"]  <- "2386"
data_for_plots_pico2$ID[data_for_plots_pico2$ID =="12B"]  <- "12"
data_for_plots_pico2$ID[data_for_plots_pico2$ID =="74"]  <- "74"
data_for_plots_pico2$ID[data_for_plots_pico2$ID =="419"]  <- "2385"
#Transforming ID in number
data_for_plots_pico2$ID <- as.numeric(data_for_plots_pico2$ID)




#creating the table in pdf
data_for_plots_pico2 %>% 
...
unique(data_for_plots_tetra2$ID) #46 IDs

data_for_plots_tetra2$ID <- as.factor(data_for_plots_tetra2$ID)

field_ID_data <- data_for_plots_tetra2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
    select(colnames(data_for_plots_tetra2), Field,  Year, State) %>% 
  left_join(summary_field_original)  %>% 
  arrange(ID)  %>% 
  rename(N_isolates =Num_original_isolates) %>% 
  select(-outlier) %>% 
  ...
  group_by (Origin, Host) %>% 
    ...
    #Transforming ID in character
    data_for_plots_tetra2$ID <- as.character(data_for_plots_tetra2$ID)
#Replacing from form ID to Inventory ID
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "78-02"] <- "2140"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "78-05"] <- "2143"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "I-20"] <- "2362"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "S-01"] <- "2320"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "54C"] <- "2453"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID == "65B"] <- "2528"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID =="419"]  <- "2385"
data_for_plots_tetra2$ID[data_for_plots_tetra2$ID =="12B"]  <- "12"
#Transforming ID in number
data_for_plots_tetra2$ID <- as.numeric(data_for_plots_tetra2$ID)

field_ID_data <- data_for_plots_TM2 %>% 
  left_join(WM, by = c(ID = "Collection_ID")) %>% 
  rename(Origin = "Origin.x", Host = "Host.x") %>% 
    select(colnames(data_for_plots_bos2), Field, molecule.s._current_season, Year, State) %>% 
  left_join(summary_field_original)  %>% 
mutate(molecule.s._current_season = na_if(molecule.s._current_season, "")) %>% 
  filter(molecule.s._current_season == "thiophanate_methyl"| is.na(molecule.s._current_season)) %>%  arrange(ID) %>% rename(Fungicide = molecule.s._current_season) %>% 
  rename(N_isolates =Num_original_isolates) %>% 
  select(-outlier)
...
ungroup()        %>% 
  ...
  mutate(MOA = "Group_7")%>% ungroup() 
 #Transforming ID in character
  data_for_plots_TM2$ID <- as.character(data_for_plots_TM2$ID)
  #Replacing from form ID to Inventory ID
  data_for_plots_TM2$ID[data_for_plots_TM2$ID == "H-01"]  <- "2220"
  data_for_plots_TM2$ID[data_for_plots_TM2$ID == "H-03"]  <- "2222"
  data_for_plots_TM2$ID[data_for_plots_TM2$ID == "H-04"]  <- "2223"
    data_for_plots_TM2$ID[data_for_plots_TM2$ID == "62-02"] <- "2098"
     data_for_plots_TM2$ID[data_for_plots_TM2$ID == "62-03"] <- "2099"
      data_for_plots_TM2$ID[data_for_plots_TM2$ID == "62-04"] <- "2100"
data_for_plots_TM2$ID[data_for_plots_TM2$ID == "78-01"] <- "2139"
      data_for_plots_TM2$ID[data_for_plots_TM2$ID == "78-02"] <- "2140"
data_for_plots_TM2$ID[data_for_plots_TM2$ID == "78-05"]  <- "2143"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="65B"]  <- "2528"
 data_for_plots_TM2$ID[data_for_plots_TM2$ID =="51C"]  <- "2449"
 data_for_plots_TM2$ID[data_for_plots_TM2$ID =="53B"]  <- "2536"
 data_for_plots_TM2$ID[data_for_plots_TM2$ID =="54C"]  <- "2453"
 data_for_plots_TM2$ID[data_for_plots_TM2$ID =="74"]  <- "74"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="I-20"]  <- "2362"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="S-01"]  <- "2320"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="W212"]  <- "2388"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="318"]  <- "2390"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="413"]  <- "2386"
data_for_plots_TM2$ID[data_for_plots_TM2$ID =="419"]  <- "2385"
#Transforming ID in numeric  
data_for_plots_TM2$ID <- as.numeric(data_for_plots_TM2$ID)  


summary_field_original <- WM %>% 
  select(Field, Collection_ID,molecule.s._current_season ) %>% 
  unique() %>% 
  group_by(Field, molecule.s._current_season) %>% 
  summarize(Num_original_isolates = n()) %>% 
  mutate(Empty_field = (Field == "")) %>% 
  filter(Empty_field == FALSE) %>% 
  select(-Empty_field)  
...
unique(data_for_plots_TM2$ID) #49 IDs

data_for_plots_TM2$ID <- as.factor(data_for_plots_TM2$ID)


#For repeated
data_for_plots_TM2$ID[duplicated(data_for_plots_TM2$ID)]
#creating the table in pdf
data_for_plots_TM2 %>% 
  kable(format = "markdown") %>%  
  write2pdf("TM_outliers.pdf")
